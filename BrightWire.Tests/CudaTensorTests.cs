...
namespace BrightWire.Tests
{
	public class CudaTensorTests
	{
		...
		public void TensorCreateFromVector()
		{
			...
			var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, DEPTH).Select(i =>
				_cpu.CreateMatrix(ROWS, COLUMNS, (j, k) => (i + 1) * (j + 1) * (k + 1))).ToList());
		...
		void _TensorIm2Col(int rows, int columns, int depth, int filterWidth, int filterHeight,
			int xStride, int yStride, bool randomData)
		{
			var normalDistribution = new Normal(0, 1);
			using var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, depth).Select(i =>
				_cpu.CreateMatrix(rows, columns,
					(j, k) => randomData ? Convert.ToSingle(normalDistribution.Sample())
						: Convert.ToSingle((i + 1) * (j + 1) * (k + 1)))).ToList());
			using var gpuTensor = _cuda.Create3DTensor(cpuTensor.Data);
		...
		void _AssertAreSame(IReadOnlyList<(int[] X, int[] Y)> cpuIndex,
			IReadOnlyList<(int[] X, int[] Y)> gpuIndex)
		{
		...
		{
			Load();
			const int FILTER_WIDTH = 2, FILTER_HEIGHT = 2, XSTRIDE = 2, YSTRIDE = 2, INPUT_WIDTH = 4,
				INPUT_HEIGHT = 4;
			var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, 2).
				Select(i => _cpu.CreateZeroMatrix(INPUT_HEIGHT, INPUT_WIDTH)).ToList()).AsIndexable();
			cpuTensor[0, 0, 0] = 1f;
			cpuTensor[0, 3, 0] = 2f;
			cpuTensor[3, 0, 0] = 3f;
			cpuTensor[3, 3, 0] = 4f;
			...
			cpuTensor[2, 1, 1] = 3f;
			cpuTensor[2, 2, 1] = 4f;
			var (cpuMaxPool, cpuIndex) =
				cpuTensor.MaxPool(FILTER_WIDTH, FILTER_HEIGHT, XSTRIDE, YSTRIDE, true);
			var cpuReverseMaxPool = cpuMaxPool.ReverseMaxPool(cpuIndex, INPUT_HEIGHT, INPUT_WIDTH,
				FILTER_WIDTH, FILTER_HEIGHT, XSTRIDE, YSTRIDE).AsIndexable();
			FloatingPointHelper.AssertEqual(cpuTensor.AsIndexable(), cpuReverseMaxPool);
			using var gpuTensor = _cuda.Create3DTensor(cpuTensor.Data);
			var (gpuMaxPool, gpuIndex) =
				gpuTensor.MaxPool(FILTER_WIDTH, FILTER_HEIGHT, XSTRIDE, YSTRIDE, true);
			FloatingPointHelper.AssertEqual(gpuMaxPool.AsIndexable(), cpuMaxPool.AsIndexable());
			using var gpuReverseMaxPool = gpuMaxPool.ReverseMaxPool(gpuIndex, INPUT_HEIGHT, INPUT_WIDTH,
				FILTER_WIDTH, FILTER_HEIGHT, XSTRIDE, YSTRIDE);
		...
		{
			Load();
			const int rows = 7, columns = 7, depth = 8, filterWidth = 2, filterHeight = 2, xStride = 2,
				yStride = 2;
			var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, depth).
				Select(i => _cpu.CreateMatrix(rows, columns, (j, k) => 0)).ToList()).AsIndexable();
			var (cpuMaxPool, cpuIndices) =
				cpuTensor.MaxPool(filterWidth, filterHeight, xStride, yStride, true);
			var cpuReverseMaxPool = cpuMaxPool.
				ReverseMaxPool(cpuIndices, rows, columns, filterWidth, filterHeight, xStride, yStride).
				AsIndexable();
			_AssertValuesAreInSamePlace(cpuReverseMaxPool, cpuTensor);
			using var gpuTensor = _cuda.Create3DTensor(cpuTensor.Data);
			var (gpuMaxPool, gpuIndices) =
				gpuTensor.MaxPool(filterWidth, filterHeight, xStride, yStride, true);
			FloatingPointHelper.AssertEqual(gpuMaxPool.AsIndexable(), cpuMaxPool.AsIndexable());
			FloatingPointHelper.AssertEqual(gpuIndices.AsIndexable(), cpuIndices.AsIndexable());
			using var gpuReverseMaxPool = gpuMaxPool.ReverseMaxPool(gpuIndices, rows, columns,
				filterWidth, filterHeight, xStride, yStride);
			FloatingPointHelper.AssertEqual(gpuReverseMaxPool.AsIndexable(), cpuReverseMaxPool);
			Cleanup();
		}

		void _TensorMaxPool(int rows, int columns, int depth, int filterWidth, int filterHeight,
			int xStride, int yStride, bool randomInit, bool calculateIndices)
		{
			var normalDistribution = new Normal(0, 1);
			var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, depth).Select(i =>
				_cpu.CreateMatrix(rows, columns,
					(j, k) => randomInit ? Convert.ToSingle(normalDistribution.Sample())
						: Convert.ToSingle((i + 1) * (j + 1) * (k + 1)))).ToList()).AsIndexable();
			var (cpuMaxPool, cpuIndices) =
				cpuTensor.MaxPool(filterWidth, filterHeight, xStride, yStride, true);
			var cpuReverseMaxPool = cpuMaxPool.
				ReverseMaxPool(cpuIndices, rows, columns, filterWidth, filterHeight, xStride, yStride).
			...
			using var gpuTensor = _cuda.Create3DTensor(cpuTensor.Data);
			var (gpuMaxPool, gpuIndices) =
				gpuTensor.MaxPool(filterWidth, filterHeight, xStride, yStride, calculateIndices);
			FloatingPointHelper.AssertEqual(gpuMaxPool.AsIndexable(), cpuMaxPool.AsIndexable());
			if (calculateIndices)
			{
				FloatingPointHelper.AssertEqual(gpuIndices.AsIndexable(), cpuIndices.AsIndexable());
		...
		void _TensorReverseIm2Col(int filterWidth, int filterHeight, int xStride, int yStride,
			int depth, int filterCount, int inputWidth, int inputHeight)
		{
			var normalDistribution = new Normal(0, 1);
			var cpuTensor = _cpu.Create3DTensor(Enumerable.Range(0, depth).Select(i =>
				_cpu.CreateMatrix(inputHeight, inputWidth,
					(j, k) => Convert.ToSingle(normalDistribution.Sample()))).ToList());
			var im2Col = cpuTensor.Im2Col(filterWidth, filterHeight, xStride, yStride);
			var cpuFilter = _cpu.CreateMatrix(depth * filterWidth * filterHeight, filterCount,
				(i, j) => (float)normalDistribution.Sample());
			...
			var cpuReverseIm2Col = outputTensor.ReverseIm2Col(cpuFilter, inputHeight, inputWidth, depth,
				filterWidth, filterHeight, xStride, yStride);
			using var gpuReverseIm2Col = gpuTensor.ReverseIm2Col(gpuFilter, inputHeight, inputWidth,
				depth, filterWidth, filterHeight, xStride, yStride);
			FloatingPointHelper.AssertEqual(gpuReverseIm2Col.AsIndexable(),
				cpuReverseIm2Col.AsIndexable());
		}
		...
		FloatMatrix _CreateMatrix(int depth, int rows, int columns,
			Func<int, int, int, float> valueProvider)
		{
			return new FloatMatrix
			{
				Row = Enumerable.Range(0, rows).Select(i => new FloatVector
				{
					Data = Enumerable.Range(0, columns).
						Select(j => valueProvider?.Invoke(i, j, depth) ?? 0f).ToArray()
				}).ToArray()
			};
		}

		FloatTensor _CreateTensor(int rows, int columns, int depth,
			Func<int, int, int, float> valueProvider)
		{
			return new FloatTensor
			{
				Matrix = Enumerable.Range(0, depth).
					Select(k => _CreateMatrix(k, rows, columns, valueProvider)).ToArray()
			};
		}
		...
		public void Tensor4DCreate()
		{
			...
			var data = Enumerable.Range(0, COUNT).Select(z => _CreateTensor(ROWS, COLUMNS, DEPTH, null)).
				ToList();
		...
		public void Tensor4DReverseMaxPool()
		{
			...
			using var gpuReverseMaxPool =
				gpuResult.Result.ReverseMaxPool(gpuResult.Indices, 4, 4, 2, 2, 2, 2);
			FloatingPointHelper.AssertEqual(cpuReverseMaxPool.AsIndexable(),
				gpuReverseMaxPool.AsIndexable());
			Cleanup();
		}
		...
		{
			Load();
			var data = Enumerable.Range(0, 5).Select(z =>
				_CreateTensor(4, 4, 2, (i, j, k) => (i + 1) * (j + 1) * (k + 1) * (z + 1))).ToList();
			var cpuTensor = _cpu.Create4DTensor(data);
		...
		public void Tensor4DToMatrix()
		{
			Load();
			var data = Enumerable.Range(0, 5).Select(z =>
				_CreateTensor(3, 4, 2, (i, j, k) => (i + 1) * (j + 1) * (k + 1) * (z + 1))).ToList();
			var cpuTensor = _cpu.Create4DTensor(data);
			var cpuMatrix = cpuTensor.ReshapeAsMatrix();
		...
		{
			Load();
			const int rows = 4, columns = 4, depth = 1, count = 1, filterWidth = 2, filterHeight = 2,
				filterCount = 1, xStride = 2, yStride = 2;
			var normalDistribution = new Normal(0, 1);
			var data = Enumerable.Range(0, count).Select(z =>
					_CreateTensor(rows, columns, depth, (i, j, k) => (float)normalDistribution.Sample())).
				ToList();
			var cpuTensor = _cpu.Create4DTensor(data);
			var cpuFilter = _cpu.CreateMatrix(depth * filterWidth * filterHeight, filterCount,
				(i, j) => (float)normalDistribution.Sample());
			...
			using var gpuFilter = _cuda.CreateMatrix(cpuFilter.AsIndexable());
			FloatingPointHelper.AssertEqual(cpuTensor.AsIndexable(), gpuTensor.AsIndexable());
			var cpuReverseIm2Col = cpuTensor.ReverseIm2Col(cpuFilter, rows, columns, depth, filterWidth,
				filterHeight, xStride, yStride);
			using var gpuReverseIm2Col = gpuTensor.ReverseIm2Col(gpuFilter, rows, columns, depth,
				filterWidth, filterHeight, xStride, yStride);
