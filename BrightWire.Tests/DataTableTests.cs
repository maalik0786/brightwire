...
namespace BrightWire.Tests
{
	public class DataTableTests
	{
		...
		public void TestIndexHydration()
		{
			...
			dataStream.Seek(0, SeekOrigin.Begin);
			var newTable2 = BrightWireProvider.CreateDataTable(dataStream);
		...
		static IDataTable _CreateComplexTable()
		{
			...
			builder.AddColumn(ColumnType.String, "string");
			for (var i = 1; i <= 10; i++)
				builder.Add(i % 2 == 0, (sbyte)i, DateTime.Now, (double)i, (float)i, i, (long)i,
					i.ToString());
		...
		public void TestDataTableAnalysis()
		{
			...
			var numericAnalysis = new[] { 1, 3, 4, 5, 6 }.Select(i => analysis[i] as INumericColumnInfo).
				ToList();
		...
		public void TestReverseNormalisation()
		{
			...
			var model = table.GetNormalisationModel(NormalisationType.FeatureScale);
			var normalised = table.Normalise(model);
			var reverseNormalised = normalised.Project(row =>
				new[] { model.ReverseNormaliseOutput(columnIndex, row.Data[columnIndex]) });
		...
		public void GetNumericColumns()
		{
			...
			builder.Add(0.2f, 0.6, "d");
			var table = builder.Build();
			var column = table.GetNumericColumns(new[] { 1 }).Select(r => _lap.CreateVector(r)).First().
				AsIndexable();
		...
		public void SelectColumns()
		{
			...
			var column = table2.GetNumericColumns(new[] { 0 }).Select(r => _lap.CreateVector(r)).First().
				AsIndexable();
			Assert.AreEqual(column[0], 1.1f);
			Assert.AreEqual(column[1], 1.5f);
		}
		...
		public void TableReverseVectorise()
		{
			...
			var reversedOutput = output.
				Select(vector => model.ReverseVectoriseOutput(vector, targetColumnType)).ToList();
