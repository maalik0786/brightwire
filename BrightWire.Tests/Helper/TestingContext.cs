...
namespace BrightWire.Tests.Helper
{
	...
	{
		public List<(IExecutionHistory, IBackpropagation)> Forward { get; } =
			new List<(IExecutionHistory, IBackpropagation)>();
		public List<(IGraphData, INode, INode)> Backward { get; } =
			new List<(IGraphData, INode, INode)>();
