...
namespace BrightWire.Tests
{
	public class DataSourceTests
	{
		...
		{
			var matrices = Enumerable.Range(0, 10).Select(j =>
				FloatMatrix.Create(Enumerable.Range(0, 10).Select(i => FloatVector.Create(GetArray(i, 10))).
					ToArray())).ToList();
		...
		{
			var tensors = Enumerable.Range(0, 10).Select(k =>
				FloatTensor.Create(Enumerable.Range(0, 10).Select(j =>
					FloatMatrix.Create(Enumerable.Range(0, 10).
						Select(i => FloatVector.Create(GetArray(i, 10))).ToArray())).ToArray())).ToList();
