	...
	partial class Program
	{
		class AAAIDocument
		{
				...
				StringTableBuilder stringTable)
			{
				...
				foreach (var item in Keyword)
				{
					...
					{
						Index = stringTable.GetIndex(item), Weight = 1f
					});
				}
				...
				return (Title,
					...
					{
						IndexList = weightedIndex.GroupBy(d => d.Index).Select(g =>
							new WeightedIndexList.WeightedIndex
							{
								Index = g.Key, Weight = g.Sum(d => d.Weight)
							}).ToArray()
					});
			}
		}

		static void _WriteClusters(string filePath, IReadOnlyList<IReadOnlyList<IVector>> clusters,
			Dictionary<IVector, AAAIDocument> documentTable)
		{
		...
		public static void TextClustering(string dataFilePath, string outputPath)
		{
			...
			dataTable.ForEach(row => docList.Add(new AAAIDocument
			{
				...
				Keyword =
					row.GetField<string>(3).Split(KEYWORD_SPLIT, StringSplitOptions.RemoveEmptyEntries).
						Select(str => str.ToLower()).ToArray(),
			...
			var lookupTable = encodings.Select(d => Tuple.Create(d, lap.CreateVector(d.Data))).
			...
			_WriteClusters(outputPath + "kmeans.txt", vectorList.KMeans(lap, allGroups.Count),
				lookupTable);
			Console.WriteLine("NNMF clustering...");
			_WriteClusters(outputPath + "nnmf.txt", vectorList.NNMF(lap, allGroups.Count, 100),
				lookupTable);
			...
			using (var randomProjection =
				lap.CreateRandomProjection((int)classificationSet.GetMaxIndex() + 1, 512))
			{
				using var projectedMatrix = randomProjection.Compute(matrix);
				var vectorList2 = Enumerable.Range(0, projectedMatrix.RowCount).
					Select(i => projectedMatrix.Row(i)).ToList();
				var lookupTable2 = vectorList2.Select((v, i) => Tuple.Create(v, vectorList[i])).
					ToDictionary(d => d.Item1, d => lookupTable[d.Item2]);
				Console.WriteLine("Kmeans clustering of random projection...");
				_WriteClusters(outputPath + "projected-kmeans.txt",
					vectorList2.KMeans(lap, allGroups.Count), lookupTable2);
			...
			var lookupTable3 = vectorList3.Select((v, i) => Tuple.Create(v, vectorList[i])).
				ToDictionary(d => (IVector)d.Item1, d => lookupTable[d.Item2]);
			Console.WriteLine("Kmeans clustering in latent document space...");
			_WriteClusters(outputPath + "latent-kmeans.txt", vectorList3.KMeans(lap, allGroups.Count),
				lookupTable3);
		}
	}
}