...
namespace BrightWire.Tests.Samples
{
	public partial class Program
	{
		...
		public static void MNIST(string dataFilesPath)
		{
			...
			var trainingData = _BuildVectors(null, graph,
				Mnist.Load(dataFilesPath + "train-labels.idx1-ubyte",
					dataFilesPath + "train-images.idx3-ubyte"));
			var testData = _BuildVectors(trainingData, graph,
				Mnist.Load(dataFilesPath + "t10k-labels.idx1-ubyte",
			...
			// create the network
			graph.Connect(engine).AddFeedForward(outputSize: 1024).Add(graph.LeakyReluActivation()).
				AddDropOut(dropOutPercentage: 0.5f).AddFeedForward(outputSize: trainingData.OutputSize).
		...
		static IDataSource _BuildVectors(IDataSource existing, GraphFactory graph,
