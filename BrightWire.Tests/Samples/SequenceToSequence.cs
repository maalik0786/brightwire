...
namespace BrightWire.Tests.Samples
{
	public partial class Program
	{
		...
		{
			var grammar = new SequenceGenerator(dictionarySize: 16, minSize: 3, maxSize: 6,
				noRepeat: true, isStochastic: false);
			var sequences = grammar.GenerateSequences().Take(1000).ToList();
			var builder = BrightWireProvider.CreateDataTableBuilder();
			builder.AddColumn(ColumnType.Matrix, "Sequence");
			...
			// build the network
			const int HIDDEN_LAYER_SIZE = 128;
			graph.Connect(engine).AddGru(HIDDEN_LAYER_SIZE)
			...
			var executionEngine = graph.CreateEngine(networkGraph);
			var output = executionEngine.Execute(testData);
			Console.WriteLine(output.Where(o => o.Target != null).
