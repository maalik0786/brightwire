...
namespace BrightWire.Cuda
{
	...
	class Gpu3DTensor : I3DTensor, IHaveDeviceMemory
	{
		...
		{
			Debug.Assert(IsValid);
			return new GpuMatrix(_cuda, _rows, _columns, _cuda.OffsetByBlock(Memory, depth, _blockSize),
				false);
		}
		...
		{
			Debug.Assert(IsValid);
			return new GpuMatrix(_cuda, _blockSize, _depth, Memory, false);
		}

		public I4DTensor ReshapeAs4DTensor(int rows, int columns)
		{
			Debug.Assert(IsValid && rows * columns == _rows);
			return new Gpu4DTensor(_cuda, rows, columns, _columns, _depth, Memory, false);
		}

		public I3DTensor AddPadding(int padding)
		{
			Debug.Assert(IsValid);
			var ret = _cuda.TensorAddPadding(Memory, _rows, _columns, _depth, 1, padding);
		...
		{
			Debug.Assert(IsValid);
			var ret = _cuda.TensorRemovePadding(Memory, _rows, _columns, _depth, 1, padding);
			return new Gpu3DTensor(_cuda, ret.Rows, ret.Columns, Depth, ret.Data, true);
		}
		...
		{
			Debug.Assert(IsValid);
			var ret = _cuda.TensorIm2Col(Memory, _rows, _columns, _depth, 1, filterWidth, filterHeight,
				xStride, yStride);
			return new GpuMatrix(_cuda, ret.Rows, ret.Columns, ret.Data, true);
		}

		public I3DTensor ReverseIm2Col(IMatrix filter, int outputRows, int outputColumns,
			int outputDepth, int filterWidth, int filterHeight, int xStride, int yStride)
		{
			Debug.Assert(IsValid);
			var filterPtr = ((IHaveDeviceMemory)filter).Memory;
			var ret = _cuda.TensorReverseIm2Col(Memory, filterPtr, _rows, _columns, _depth, 1, outputRows,
				outputColumns, outputDepth, filterWidth, filterHeight, xStride, yStride);
		...
		public I3DTensor ReverseMaxPool(I3DTensor indices, int outputRows, int outputColumns,
			int filterWidth, int filterHeight, int xStride, int yStride)
		{
			Debug.Assert(IsValid);
			var indicesPtr = ((IHaveDeviceMemory)indices).Memory;
			var ret = _cuda.TensorReverseMaxPool(Memory, indicesPtr, _rows, _columns, _depth, 1,
