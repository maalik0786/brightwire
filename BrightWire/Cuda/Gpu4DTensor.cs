...
namespace BrightWire.Cuda
{
	...
	class Gpu4DTensor : I4DTensor, IHaveDeviceMemory
	{
		...
		public Gpu4DTensor(CudaProvider provider, int rows, int columns, int depth, int count,
		...
		public IMatrix ReshapeAsMatrix() => new GpuMatrix(_cuda, _blockSize, _count, Memory, false);
		...
		public IVector ReshapeAsVector() => new GpuVector(_cuda, Memory, false);
		...
		{
			return new Gpu3DTensor(_cuda, _rows, _columns, _depth,
				_cuda.OffsetByBlock(Memory, index, _blockSize), false);
		}
		...
		{
			Debug.Assert(IsValid);
			var ret = _cuda.TensorAddPadding(Memory, _rows, _columns, _depth, _count, padding);
		...
		{
			Debug.Assert(IsValid);
			var ret = _cuda.TensorRemovePadding(Memory, _rows, _columns, _depth, _count, padding);
			return new Gpu4DTensor(_cuda, ret.Rows, ret.Columns, _depth, _count, ret.Data, true);
		}

		public (I4DTensor Result, I4DTensor Indices) MaxPool(int filterWidth, int filterHeight,
			int xStride, int yStride, bool saveIndices)
		{
			Debug.Assert(IsValid);
			var maxPool = _cuda.TensorMaxPool(Memory, _rows, _columns, _depth, _count, filterWidth,
				filterHeight, xStride, yStride, saveIndices);
			var ret = new Gpu4DTensor(_cuda, maxPool.Rows, maxPool.Columns, _depth, _count, maxPool.Data,
				true);
			var indices = saveIndices ? new Gpu4DTensor(_cuda, maxPool.Rows, maxPool.Columns, _depth,
				_count, maxPool.Indices, true) : null;
			return (ret, indices);
		}

		public I4DTensor ReverseMaxPool(I4DTensor indices, int outputRows, int outputColumns,
			int filterWidth, int filterHeight, int xStride, int yStride)
		{
			Debug.Assert(IsValid);
			var indicesPtr = ((IHaveDeviceMemory)indices).Memory;
			var ret = _cuda.TensorReverseMaxPool(Memory, indicesPtr, _rows, _columns, _depth, _count,
		...
		{
			var ret = _cuda.TensorIm2Col(Memory, _rows, _columns, _depth, _count, filterWidth,
				filterHeight, xStride, yStride);
			return new Gpu3DTensor(_cuda, ret.Rows, ret.Columns, ret.Depth, ret.Data, true);
		}

		public I4DTensor ReverseIm2Col(IMatrix filter, int outputRows, int outputColumns,
			int outputDepth, int filterWidth, int filterHeight, int xStride, int yStride)
		{
			var filterPtr = ((IHaveDeviceMemory)filter).Memory;
			var ret = _cuda.TensorReverseIm2Col(Memory, filterPtr, _rows, _columns, _depth, _count,
				outputRows, outputColumns, outputDepth, filterWidth, filterHeight, xStride, yStride);
