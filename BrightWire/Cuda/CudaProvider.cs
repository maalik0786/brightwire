	...
	internal class CudaProvider : ILinearAlgebraProvider, IGpuLinearAlgebraProvider
	{
		...
		class KernelExecution
		{
			...
			public void Run(uint sharedMemSize, object[] param)
			{
				...
				var result = DriverAPINativeMethods.Launch.cuLaunchKernel(_function, _block.x, _block.y,
					_block.z, _thread.x, _thread.y, _thread.z, sharedMemSize, new CUstream(), paramList,
					null);
		...
		class KernelModule
		{
			...
			public CUfunction LoadFunction(string name)
			{
				...
				if (DriverAPINativeMethods.ModuleManagement.cuModuleGetFunction(ref ret, _module, name) !=
					CUResult.Success)
			...
			public KernelExecution CreateExecution(CUfunction function, dim3 block, dim3 thread) =>
				new KernelExecution(function, block, thread);
		}

		readonly Lazy<CudaSolveDense> _solver = new Lazy<CudaSolveDense>();
		readonly KernelModule _kernel;
		readonly DeviceMemory _cache;
		readonly CUfunction _pointwiseMultiply, _addInPlace, _subtractInPlace, _addToEachRow,
			_addToEachColumn, _tanh, _tanhDerivative, _sigmoid, _sigmoidDerivative, _sumRows, _relu,
			_reluDerivative, _leakyRelu, _leakyReluDerivative, _memClear, _sumColumns, _pointwiseDivide,
			_sqrt, _findMinAndMax, _findSum, _findStdDev, _constrain, _pow, _diagonal, _l1Regularisation,
			_pointwiseDivideRows, _pointwiseDivideColumns, _splitRows, _splitColumns, _concatRows,
			_concatColumns, _euclideanDistance, _manhattanDistance, _cosineDistance, _abs, _normalise,
			_softmaxVector,
			//_multiEuclidean,
			//_multiManhattan,
			_multiCosine, _log, _vectorAdd, _vectorCopyRandom, _copyToMatrixColumns, _copyToMatrixRows,
			_tensorAddPadding, _tensorRemovePadding, _tensorIm2Col, _softmaxDerivative, _reverse,
			_rotateInPlace, _tensorMaxPool, _tensorReverseMaxPool, _tensorReverseIm2Col, _isFinite,
			_calculateDistance;
		readonly ConcurrentDictionary<CUfunction, (int BlockSize, int MinGridSize)> _blockSize =
			new ConcurrentDictionary<CUfunction, (int BlockSize, int MinGridSize)>();
		bool _disposed;
		...
		public CudaProvider(string cudaKernelPath, bool stochastic, int memoryCacheSize)
		{
			...
			_cache = new DeviceMemory(Context, memoryCacheSize);
			_kernel = new KernelModule(Context, cudaKernelPath);
			Blas = new CudaBlas(AtomicsMode.Allowed);
			Context.SetCurrent();
		...
		public ILinearAlgebraProvider NumericsProvider { get; } =
			BrightWireProvider.CreateLinearAlgebra();
		public bool IsStochastic { get; }
		public bool IsGpu => true;
		internal CudaContext Context { get; }
		internal CudaBlas Blas { get; }
		public CudaSolveDense Solver => _solver.Value;
		public long TotalMemory => Context.GetTotalDeviceMemorySize();
		public long FreeMemory => Context.GetFreeDeviceMemorySize();

		public void Register(IDisposable disposable) => _cache.Add(disposable);

		int _GetBlockCount(int size, int blockSize) => ((size / blockSize) + 1);
		...
		internal static void CheckForError(CUResult result)
		{
			if (result != CUResult.Success)
			{
				...
				if (DriverAPINativeMethods.ErrorHandling.cuGetErrorName(result, ref errorNamePtr) ==
					CUResult.Success && errorNamePtr != IntPtr.Zero)
					errorName = Marshal.PtrToStringUni(errorNamePtr);
				if (DriverAPINativeMethods.ErrorHandling.cuGetErrorString(result, ref errorDescPtr) ==
					CUResult.Success && errorDescPtr != IntPtr.Zero)
		...
		void _Invoke(CUfunction function, int size, params object[] param)
		{
			if (!_blockSize.TryGetValue(function, out var data))
			{
				...
				DriverAPINativeMethods.Occupancy.cuOccupancyMaxPotentialBlockSize(ref minGridSize,
					ref blockSize, function, bs => 0, 0, 0);
		...
		void _InvokeWithSharedMemory(CUfunction function, int size, uint sharedMemorySize,
			params object[] param)
		{
			var gridSize = _GetBlockCount(size, BLOCK_DIM2);
			var execution = _kernel.CreateExecution(function, gridSize, BLOCK_DIM2);
		...
		void _Invoke2(CUfunction function, int rows, int columns, params object[] param)
		{
			if (!_blockSize.TryGetValue(function, out var data))
			{
				...
				DriverAPINativeMethods.Occupancy.cuOccupancyMaxPotentialBlockSize(ref minGridSize,
					ref blockSize, function, bs => 0, 0, 0);
				_blockSize.TryAdd(function,
					data = (Convert.ToInt32(Math.Pow(blockSize, 1.0 / 2)), minGridSize));
			}

			int gridSizeRows = (rows + data.BlockSize - 1) / data.BlockSize;
			int gridSizeCols = (columns + data.BlockSize - 1) / data.BlockSize;
			var execution = _kernel.CreateExecution(function, new dim3(gridSizeRows, gridSizeCols),
				new dim3(data.BlockSize, data.BlockSize));
			execution.Run(0, param);
		}
		...
		void _Invoke3(CUfunction function, int rows, int columns, int depth, params object[] param)
		{
			if (!_blockSize.TryGetValue(function, out var data))
			{
				...
				DriverAPINativeMethods.Occupancy.cuOccupancyMaxPotentialBlockSize(ref minGridSize,
					ref blockSize, function, bs => 0, 0, 0);
				_blockSize.TryAdd(function,
					data = (Convert.ToInt32(Math.Pow(blockSize, 1.0 / 3)), minGridSize));
			}

			int gridSizeRows = (rows + data.BlockSize - 1) / data.BlockSize;
			int gridSizeCols = (columns + data.BlockSize - 1) / data.BlockSize;
			int gridSizeDepth = (depth + data.BlockSize - 1) / data.BlockSize;
			var execution = _kernel.CreateExecution(function,
				new dim3(gridSizeRows, gridSizeCols, gridSizeDepth),
				new dim3(data.BlockSize, data.BlockSize, data.BlockSize));
			execution.Run(0, param);
		}
		...
		internal bool IsFinite(IDeviceMemoryPtr a, int size)
		{
			...
			try
			{
				...
				var sum = Blas.AbsoluteSum(ret.DeviceVariable, 1);
		...
		internal void AddInPlace(IDeviceMemoryPtr a, IDeviceMemoryPtr b, int size, float coefficient1,
			float coefficient2)
		{
			_Invoke(_addInPlace, size, a.DevicePointer, b.DevicePointer, size, coefficient1,
				coefficient2);
		}

		internal void SubtractInPlace(IDeviceMemoryPtr a, IDeviceMemoryPtr b, int size,
			float coefficient1, float coefficient2)
		{
			_Invoke(_subtractInPlace, size, a.DevicePointer, b.DevicePointer, size, coefficient1,
				coefficient2);
		}

		internal void AddToEachRow(IDeviceMemoryPtr matrix, IDeviceMemoryPtr vector, int rows,
			int columns)
		{
			_Invoke2(_addToEachRow, rows, columns, matrix.DevicePointer, vector.DevicePointer, rows,
				columns);
		}

		internal void AddToEachColumn(IDeviceMemoryPtr matrix, IDeviceMemoryPtr vector, int rows,
			int columns)
		{
			_Invoke2(_addToEachColumn, rows, columns, matrix.DevicePointer, vector.DevicePointer, rows,
				columns);
		}
		...
		internal IDeviceMemoryPtr VectorCopy(IDeviceMemoryPtr a, int size, int[] indexList)
		{
			...
			_Invoke(_vectorCopyRandom, retSize, a.DevicePointer, ret.DevicePointer,
				indexGpu.DevicePointer, retSize);
			return ret;
		}
		...
		internal (float Min, float Max) FindMinAndMax(IDeviceMemoryPtr a, int size)
		{
			if (size > 0)
			{
				...
				while (size > BLOCK_DIM2)
				{
					...
					try
					{
						_InvokeManual(_findMinAndMax, size, ptr.DevicePointer, size, minBlock.DevicePointer,
							maxBlock.DevicePointer);
						if (ptr != a)
							ptr.Free();
						size = bufferSize * 2;
						ptr = Allocate(size);
						ptr.DeviceVariable.CopyToDevice(minBlock.DeviceVariable, 0, 0, bufferSize * FLOAT_SIZE);
						ptr.DeviceVariable.CopyToDevice(maxBlock.DeviceVariable, 0, bufferSize * FLOAT_SIZE,
							bufferSize * FLOAT_SIZE);
					}
					finally
					{
						minBlock.Free();
		...
		internal float CosineDistance(IDeviceMemoryPtr a, IDeviceMemoryPtr b, int size)
		{
			...
			try
			{
				_Invoke(_cosineDistance, size, a.DevicePointer, b.DevicePointer, aaDevice.DevicePointer,
					abDevice.DevicePointer, bbDevice.DevicePointer, size);
				...
				if (aa.Equals(0f))
					...
					return 1f - (ab / (float)Math.Sqrt(aa) / (float)Math.Sqrt(bb));
			}
			finally
			{
				aaDevice.Free();
		...
		{
			_Invoke2(_pointwiseDivideRows, rows, columns, a.DevicePointer, b.DevicePointer, rows,
				columns);
		}

		internal void PointwiseDivideColumns(IDeviceMemoryPtr a, IDeviceMemoryPtr b, int rows,
			int columns)
		{
			_Invoke2(_pointwiseDivideColumns, rows, columns, a.DevicePointer, b.DevicePointer, rows,
				columns);
		}

		internal void SplitRows(IDeviceMemoryPtr a, IDeviceMemoryPtr b, IDeviceMemoryPtr c, int rows,
			int columns, int position)
		{
			_Invoke2(_splitRows, rows, columns, a.DevicePointer, b.DevicePointer, c.DevicePointer, rows,
				columns, position);
		}

		internal void SplitColumns(IDeviceMemoryPtr a, IDeviceMemoryPtr b, IDeviceMemoryPtr c, int rows,
			int columns, int position)
		{
			_Invoke2(_splitColumns, rows, columns, a.DevicePointer, b.DevicePointer, c.DevicePointer,
				rows, columns, position);
		}

		internal void ConcatRows(IDeviceMemoryPtr a, IDeviceMemoryPtr b, IDeviceMemoryPtr c, int rows,
			int columns, int leftColumnCount)
		{
			_Invoke2(_concatRows, rows, columns, a.DevicePointer, b.DevicePointer, c.DevicePointer, rows,
				columns, leftColumnCount);
		}

		internal void ConcatColumns(IDeviceMemoryPtr a, IDeviceMemoryPtr b, IDeviceMemoryPtr c,
			int rows, int columns, int topRowCount, int bottomRowCount)
		{
			_Invoke2(_concatColumns, rows, columns, a.DevicePointer, b.DevicePointer, c.DevicePointer,
				rows, columns, topRowCount, bottomRowCount);
		}
		...
		internal (IDeviceMemoryPtr Data, int Rows, int Columns) TensorAddPadding(
			IDeviceMemoryPtr tensor, int rows, int columns, int depth, int count, int padding)
		{
			...
			_Invoke(_tensorAddPadding, ret.Size, ret.Size, tensor.DevicePointer, ret.DevicePointer, rows,
				columns, depth, count, outputRows, outputColumns, padding);
			...
			IDeviceMemoryPtr tensor, int rows, int columns, int depth, int count, int padding)
		{
			...
			_Invoke(_tensorRemovePadding, size, size, tensor.DevicePointer, ret.DevicePointer, rows,
				columns, depth, count, outputRows, outputColumns, padding);
			return (ret, outputRows, outputColumns);
		}
		...
		internal (IDeviceMemoryPtr Data, IDeviceMemoryPtr Indices, int Rows, int Columns) TensorMaxPool(
			IDeviceMemoryPtr tensor, int rows, int columns, int depth, int count, int filterWidth,
			int filterHeight, int xStride, int yStride, bool saveIndices)
		{
			...
			var convolutions =
				ConvolutionHelper.Default(columns, rows, filterWidth, filterHeight, xStride, yStride);
			var size = convolutions.Count * depth * count;
			using var convolutionData = new ConvolutionsData(this, convolutions);
			_Invoke(_tensorMaxPool, size, size, tensor.DevicePointer, ret.DevicePointer,
				saveIndices ? indices.DevicePointer : new CUdeviceptr(), convolutionData.X.DevicePointer,
				convolutionData.Y.DevicePointer, convolutions.Count, rows, columns, depth, count,
				outputRows, outputColumns, filterWidth, filterHeight, xStride, yStride,
				saveIndices ? 1 : 0);
			return (ret, indices, outputRows, outputColumns);
		}

		internal IDeviceMemoryPtr TensorReverseMaxPool(IDeviceMemoryPtr tensor,
			IDeviceMemoryPtr indices, int rows, int columns, int depth, int count, int outputRows,
			int outputColumns, int filterWidth, int filterHeight, int xStride, int yStride)
		{
			var ret = Allocate(outputRows * outputColumns * depth * count, true);
			var size = rows * columns * depth * count;
			_Invoke(_tensorReverseMaxPool, size, size, tensor.DevicePointer, indices.DevicePointer,
				ret.DevicePointer, rows, columns, depth, count, outputRows, outputColumns, filterWidth,
				filterHeight, xStride, yStride);
			return ret;
		}
			...
			int filterHeight, int xStride, int yStride)
		{
			...
			using var convolutionData = new ConvolutionsData(this, convolutions);
			_Invoke(_tensorIm2Col, ret.Size, ret.Size, tensor.DevicePointer, ret.DevicePointer,
				convolutionData.X.DevicePointer, convolutionData.Y.DevicePointer, rows, columns, depth,
				count, outputRows, outputColumns, convolutionData.Count, filterWidth, filterHeight, xStride,
				yStride);
			return (ret, outputRows, outputColumns, count);
		}

		internal (IDeviceMemoryPtr Data, int Rows, int Columns, int Depth, int Count)
			TensorReverseIm2Col(IDeviceMemoryPtr tensor, IDeviceMemoryPtr filters, int rows, int columns,
				int depth, int count, int outputRows, int outputColumns, int outputDepth, int filterWidth,
				int filterHeight, int xStride, int yStride)
		{
			var ret = Allocate(outputRows * outputColumns * outputDepth * count, true);
			using (var convolutions = new ConvolutionsData(this,
				ConvolutionHelper.Default(outputColumns, outputRows, filterWidth, filterHeight, xStride,
					yStride)))
			{
				var size = depth * convolutions.Count * filterHeight * filterWidth * outputDepth * count;
				_Invoke(_tensorReverseIm2Col, size, size, tensor.DevicePointer, filters.DevicePointer,
					ret.DevicePointer, convolutions.X.DevicePointer, convolutions.Y.DevicePointer, rows,
					columns, depth, count, convolutions.Count, filterWidth, filterHeight, xStride, yStride,
					outputRows, outputColumns, outputDepth);
			}
			...
			IReadOnlyList<IVector> compareTo, DistanceMetric distanceMetric)
		{
			if (!(distanceMetric == DistanceMetric.Euclidean ||
				distanceMetric == DistanceMetric.Manhattan || distanceMetric == DistanceMetric.Cosine))
			...
			{
				using var compareToPtr =
					new PtrToDeviceMemoryList(compareTo.Cast<IHaveDeviceMemory>().ToList());
				if (distanceMetric == DistanceMetric.Cosine)
				{
					var aa = Allocate(rows * columns, true);
					var bb = Allocate(rows * columns, true);
					_Invoke3(_multiCosine, size, columns, rows, vectorPtr.DevicePointer,
						compareToPtr.DevicePointer, aa.DevicePointer, ret.DevicePointer, bb.DevicePointer, rows,
						columns, size);
				...
				_Invoke3(_calculateDistance, size, columns, rows, vectorPtr.DevicePointer,
					compareToPtr.DevicePointer, ret.DevicePointer, rows, columns, size, (int)distanceMetric);
			}
		...
		public IMatrix CreateMatrixFromRows(IReadOnlyList<IVector> vectorRows)
		{
			...
			{
				devicePtr.CopyToDevice(vectorRows.Cast<IHaveDeviceMemory>().
					Select(d => d.Memory.DevicePointer).ToArray());
				_Invoke2(_copyToMatrixRows, rows, columns, devicePtr.DevicePointer, ret.DevicePointer, rows,
					columns);
			}
		...
		public IMatrix CreateMatrixFromColumns(IReadOnlyList<IVector> vectorColumns)
		{
			...
			{
				devicePtr.CopyToDevice(vectorColumns.Cast<IHaveDeviceMemory>().
					Select(d => d.Memory.DevicePointer).ToArray());
				_Invoke2(_copyToMatrixColumns, rows, columns, devicePtr.DevicePointer, ret.DevicePointer,
					rows, columns);
			}
		...
		public I3DTensor Create3DTensor(IReadOnlyList<IMatrix> matrices)
		{
			...
			{
				devicePtr.CopyToDevice(matrices.Cast<IHaveDeviceMemory>().
					Select(d => d.Memory.DevicePointer).ToArray());
				_Invoke2(_copyToMatrixColumns, outputRows, outputColumns, devicePtr.DevicePointer,
					ret.DevicePointer, outputRows, outputColumns);
			}

			return new Gpu3DTensor(this, rows, columns, depth, ret, true);
		}

		public I4DTensor Create4DTensor(int rows, int columns, int depth, int count,
		...
		public I4DTensor Create4DTensor(IReadOnlyList<I3DTensor> tensors)
		{
			...
			{
				devicePtr.CopyToDevice(tensors.Cast<IHaveDeviceMemory>().
					Select(d => d.Memory.DevicePointer).ToArray());
				_Invoke2(_copyToMatrixColumns, outputRows, outputColumns, devicePtr.DevicePointer,
					ret.DevicePointer, outputRows, outputColumns);
			}
		...
		public I4DTensor Create4DTensor(IReadOnlyList<FloatTensor> tensors)
		{
			...
			var ret = new Gpu4DTensor(this, first.RowCount, first.ColumnCount, first.Depth, tensors.Count,
				data, true);
		...
		public IDeviceMemoryPtr Offset(IDeviceMemoryPtr ptr, SizeT offsetByElements, SizeT size)
		{
			...
			return new PtrToMemory(Context, ptr, new CUdeviceptr(offsetPtr), size * FLOAT_SIZE);
		}
