	...
	class GpuVector : IVector, IHaveDeviceMemory
	{
		...
		public FloatVector Data
		{
			get
			{
				...
				return new FloatVector { Data = data };
			}
			set
			{
				Debug.Assert(IsValid);
		...
		public void CopyFrom(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			Memory.CopyToDevice(other.Memory);
		}
		...
		public float DotProduct(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			return _cuda.Blas.Dot(Memory.DeviceVariable, 1, other.Memory.DeviceVariable, 1);
		}

		public IVector GetNewVectorFromIndexes(IReadOnlyList<int> indices)
		{
			Debug.Assert(IsValid);
			var data = _cuda.VectorCopy(Memory, Count, indices.ToArray());
			return new GpuVector(_cuda, data, true);
		}
		...
		{
			Debug.Assert(IsValid);
			return new GpuVector(_cuda, _cuda.Abs(Memory, Count), true);
		}

		public IVector Log()
		{
			Debug.Assert(IsValid);
			return new GpuVector(_cuda, _cuda.Log(Memory, Count), true);
		}

		public IVector Sigmoid()
		{
			Debug.Assert(IsValid);
			return new GpuVector(_cuda, _cuda.Sigmoid(Memory, Count), true);
		}
		...
		public float L1Norm()
		{
			...
			return _cuda.SumValues(abs.Memory, Count);
		}

		public float L2Norm()
		{
			Debug.Assert(IsValid);
			return _cuda.Blas.Norm2(Memory.DeviceVariable, 1);
		}

		public int MaximumIndex()
		{
			Debug.Assert(IsValid);
			return _cuda.Blas.Max(Memory.DeviceVariable, 1) - 1;
		}

		public int MinimumIndex()
		{
			Debug.Assert(IsValid);
			return _cuda.Blas.Min(Memory.DeviceVariable, 1) - 1;
		}
		...
		public IVector PointwiseMultiply(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			return new GpuVector(_cuda, _cuda.PointwiseMultiply(Memory, other.Memory, Count), true);
		}

		public IVector Sqrt()
		{
			Debug.Assert(IsValid);
			var ret = _cuda.Sqrt(Memory, Count, 1e-8f);
			return new GpuVector(_cuda, ret, true);
		}
		...
		public void SubtractInPlace(IVector vector, float coefficient1 = 1, float coefficient2 = 1)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			_cuda.SubtractInPlace(Memory, other.Memory, Count, coefficient1, coefficient2);
		}

		public IMatrix ReshapeAsColumnMatrix()
		{
			Debug.Assert(IsValid);
			return new GpuMatrix(_cuda, Count, 1, Memory, false);
		}

		public IMatrix ReshapeAsRowMatrix()
		{
			Debug.Assert(IsValid);
			return new GpuMatrix(_cuda, 1, Count, Memory, false);
		}
		...
		public float EuclideanDistance(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			return _cuda.EuclideanDistance(Memory, other.Memory, Count);
		}
		...
		public float CosineDistance(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			return _cuda.CosineDistance(Memory, other.Memory, Count);
		...
		public float ManhattanDistance(IVector vector)
		{
			...
			var other = (GpuVector)vector;
			Debug.Assert(other.Count == Count);
			return _cuda.ManhattanDistance(Memory, other.Memory, Count);
		}
		...
		{
			Debug.Assert(IsValid);
			return _cuda.FindMinAndMax(Memory, Count);
		}

		public float Average()
		{
			Debug.Assert(IsValid);
			return _cuda.SumValues(Memory, Count) / Count;
		}

		public float StdDev(float? mean)
		{
			Debug.Assert(IsValid);
			return _cuda.FindStdDev(Memory, Count, mean ?? Average());
		}
		...
		public IVector Softmax()
		{
			Debug.Assert(IsValid);
			var minMax = GetMinMax();
			var softmax = _cuda.SoftmaxVector(Memory, Count, minMax.Max);
		...
		{
			Debug.Assert(IsValid);
			_cuda.VectorAdd(Memory, Count, scalar);
		}
		...
		public IReadOnlyList<IVector> Split(int blockCount)
		{
			...
			{
				var ptr2 = _cuda.OffsetByBlock(Memory, i, blockSize);
		...
		{
			Debug.Assert(IsValid);
			var ret = _cuda.VectorSoftmaxDerivative(Memory, Count);
		...
		{
			Debug.Assert(IsValid);
			_cuda.RotateInPlace(Memory, Count, blockCount);
		}

		public IVector Reverse()
		{
			Debug.Assert(IsValid);
			var ret = _cuda.Reverse(Memory, Count);
			return new GpuVector(_cuda, ret, true);
		}
		...
		{
			Debug.Assert(IsValid);
			return Memory.DeviceVariable[index];
		}

		public void SetAt(int index, float value)
		{
			Debug.Assert(IsValid);
			Memory.DeviceVariable[index] = value;
		}

		public bool IsEntirelyFinite()
		{
			return _cuda.IsFinite(Memory, Memory.Size);
		}

		public IMatrix ReshapeAsMatrix(int rows, int columns)
		{
			Debug.Assert(IsValid && rows * columns == Memory.Size);
			return new GpuMatrix(_cuda, rows, columns, Memory, false);
		}

		public I3DTensor ReshapeAs3DTensor(int rows, int columns, int depth)
		{
			Debug.Assert(IsValid && rows * columns * depth == Memory.Size);
			return new Gpu3DTensor(_cuda, rows, columns, depth, Memory, false);
		}

		public I4DTensor ReshapeAs4DTensor(int rows, int columns, int depth, int count)
		{
			Debug.Assert(IsValid && rows * columns * depth * count == Memory.Size);
			return new Gpu4DTensor(_cuda, rows, columns, depth, count, Memory, false);
		}
	}
}