...
namespace BrightWire.Cuda
{
	...
	public static class BrightWireGpuProvider
	{
		...
		public static ILinearAlgebraProvider CreateLinearAlgebra(bool stochastic = true,
			int memoryCacheSize = 512 * 1048576, string cudaKernelPath = null)
		{
			var path = cudaKernelPath ?? GetKernelPath();
			if (!File.Exists(path))
				throw new FileNotFoundException(
					$"Could not find cuda kernel at: {path}. Is the \\cuda\\brightwire.ptx file set to 'Copy to Output Directory'?");
		...
		public static string GetKernelPath()
		{
			var assemblyLocation =
				Path.GetDirectoryName((Assembly.GetEntryAssembly() ?? Assembly.GetExecutingAssembly()).
					Location);
