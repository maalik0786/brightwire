...
namespace BrightWire.LinearAlgebra
{
	...
	class CpuVector : IIndexableVector
	{
		...
		public float this[int index] { get => _vector[index]; set => _vector[index] = value; }
		...
		public void SubtractInPlace(IVector vector, float coefficient1 = 1.0f,
		...
		{
			get => new FloatVector { Data = _vector.ToArray() };
			set
			{
				if (value.Data != null)
				{
		...
		public IVector Abs()
		{
			return new CpuVector(DenseVector.Create(_vector.Count,
				i => Convert.ToSingle(Math.Abs(_vector[i]))));
		}

		public IVector Sqrt()
		{
			return new CpuVector(DenseVector.Create(_vector.Count,
				i => Convert.ToSingle(Math.Sqrt(_vector[i] + 1e-8f))));
		}
		...
		{
			return new CpuVector(DenseVector.Create(Count + data.Count,
				i => i < Count ? _vector[i] : data[i - Count]));
		}
		...
		{
			return new CpuMatrix(DenseMatrix.Build.Dense(Count, Count,
				(x, y) => x == y ? this[x] * (1 - this[x]) : -this[x] * this[y]));
		}
		...
		public IVector CosineDistance(IReadOnlyList<IVector> data, ref float[] dataNorm)
		{
			...
			for (var i = 0; i < data.Count; i++)
				ret[i] = Convert.ToSingle(1d - DotProduct(data[i]) /
					(Math.Sqrt(norm) * Math.Sqrt(dataNorm[i])));
			return new CpuVector(DenseVector.Create(data.Count, i => ret[i]));
		}
		...
		public I3DTensor ReshapeAs3DTensor(int rows, int columns, int depth)
		{
			...
			if (depth > 1)
			{
				...
				var matrixList = slice.Select(part => part.ReshapeAsMatrix(rows, columns).AsIndexable()).
					ToList();
		...
		public I4DTensor ReshapeAs4DTensor(int rows, int columns, int depth, int count)
		{
			...
			if (count > 1)
			{
				...
				var tensorList = slice.
					Select(part => part.ReshapeAs3DTensor(rows, columns, depth).AsIndexable()).ToList();
