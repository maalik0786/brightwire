...
namespace BrightWire.LinearAlgebra
{
	...
	class CpuProvider : ILinearAlgebraProvider
	{
		...
		{
			return new Cpu3DTensor(Enumerable.Range(0, depth).
				Select(i => CreateMatrix(rows, columns, setToZero).AsIndexable()).ToList());
		}

		public I4DTensor Create4DTensor(int rows, int columns, int depth, int count,
			bool setToZero = false)
		{
			return new Cpu4DTensor(Enumerable.Range(0, count).
				Select(i => Create3DTensor(rows, columns, depth, setToZero).AsIndexable()).ToList());
		}
		...
		{
			return new Cpu4DTensor(data.Select(this.Create3DTensor).Select(t => t.AsIndexable()).
				ToList());
		}
