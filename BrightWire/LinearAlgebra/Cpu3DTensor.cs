...
namespace BrightWire.LinearAlgebra
{
	...
	class Cpu3DTensor : IIndexable3DTensor
	{
		...
		public Cpu3DTensor(IReadOnlyList<IIndexableMatrix> matrixList)
		{
			...
			Debug.Assert(matrixList.All(m =>
				m.RowCount == first.RowCount && m.ColumnCount == first.ColumnCount));
			RowCount = first.RowCount;
			ColumnCount = first.ColumnCount;
			Depth = matrixList.Count;
			var offset = 0;
			var rowSize = RowCount * ColumnCount;
			var data = new float[rowSize * Depth];
			...
			_data = new CpuMatrix(DenseMatrix.Build.Dense(rowSize, Depth, data));
		}
		...
		{
			return $"3D tensor, rows:{RowCount} columns:{ColumnCount} depth:{Depth}";
		}
		...
		int _RowIndex(int row, int column) => column * RowCount + row;

		public FloatTensor Data
		{
			get { return FloatTensor.Create(Matrix.Select(m => m.Data).ToArray()); }
			set
			{
				var matrixList = value.Matrix;
				var matrixCount = matrixList.Length;
				for (var k = 0; k < matrixCount && k < Depth; k++)
				{
		...
		public IReadOnlyList<IIndexableMatrix> Matrix =>
			_data.Columns.Select(c => c.ReshapeAsMatrix(RowCount, ColumnCount).AsIndexable()).ToList();
		public int Depth { get; }
		public int RowCount { get; }
		public int ColumnCount { get; }

		public IMatrix GetMatrixAt(int depth) =>
			_data.Column(depth).ReshapeAsMatrix(RowCount, ColumnCount);
			...
			int xStride, int yStride, bool saveIndices)
		{
			...
			var convolutions = ConvolutionHelper.Default(ColumnCount, RowCount, filterWidth, filterHeight,
				xStride, yStride);
			for (var k = 0; k < Depth; k++)
			{
				var indices = saveIndices ? new CpuMatrix(DenseMatrix.Create(newRows, newColumns, 0f))
					: null;
		...
		public I3DTensor ReverseMaxPool(I3DTensor indexList, int outputRows, int outputColumns,
		...
		public I3DTensor ReverseIm2Col(IMatrix filterMatrix, int outputRows, int outputColumns,
			int outputDepth, int filterWidth, int filterHeight, int xStride, int yStride)
		{
			var convolutions = ConvolutionHelper.Default(outputColumns, outputRows, filterWidth,
				filterHeight, xStride, yStride);
			var output = Enumerable.Range(0, outputDepth).
				Select(i => DenseMatrix.Create(outputRows, outputColumns, 0f)).ToList();
			for (var k = 0; k < Depth; k++)
			{
				var slice = GetMatrixAt(k).AsIndexable();
				var filters = filterMatrix.Column(k).Split(outputDepth).Select(v => v.AsIndexable()).
					ToList();
				foreach (var (cx, cy) in convolutions)
				{
