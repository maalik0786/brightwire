...
namespace BrightWire.LinearAlgebra
{
	...
	class CpuMatrix : IIndexableMatrix
	{
		...
		public void SubtractInPlace(IMatrix matrix, float coefficient1 = 1.0f,
		...
		public FloatMatrix Data
		{
			get
			{
				...
				for (var i = 0; i < _matrix.RowCount; i++)
				{
					...
					ret.Row[i] = new FloatVector { Data = row };
				}
		...
		{
			get { return _matrix.EnumerateRows().Select(v => new CpuVector(v)); }
		}

		public IEnumerable<IIndexableVector> Columns
		{
			get { return _matrix.EnumerateColumns().Select(v => new CpuVector(v)); }
		}

		public IMatrix GetNewMatrixFromRows(IReadOnlyList<int> rowIndexes)
		{
			return new CpuMatrix(DenseMatrix.Create(rowIndexes.Count, ColumnCount,
				(x, y) => _matrix[rowIndexes[x], y]));
		}

		public IMatrix GetNewMatrixFromColumns(IReadOnlyList<int> columnIndexes)
		{
			return new CpuMatrix(DenseMatrix.Create(RowCount, columnIndexes.Count,
				(x, y) => _matrix[x, columnIndexes[y]]));
		}
		...
		public (IMatrix Left, IMatrix Right) SplitAtColumn(int columnIndex)
		{
			...
			var ret2 = DenseMatrix.Create(RowCount, ColumnCount - columnIndex,
				(x, y) => this[x, columnIndex + y]);
		...
		public (IMatrix Top, IMatrix Bottom) SplitAtRow(int rowIndex)
		{
			...
			var ret2 = DenseMatrix.Create(RowCount - rowIndex, ColumnCount,
				(x, y) => this[rowIndex + x, y]);
			return (new CpuMatrix(ret1), new CpuMatrix(ret2));
		}
		...
		public IMatrix Sqrt(float valueAdjustment = 1e-8f)
		{
			return new CpuMatrix(
				(DenseMatrix)_matrix.Map(v => Convert.ToSingle(Math.Sqrt(v + valueAdjustment))));
		}
		...
		public I4DTensor ReshapeAs4DTensor(int rows, int columns, int depth)
		{
			...
			for (var i = 0; i < ColumnCount; i++)
				list.Add(new Cpu3DTensor(Column(i).Split(depth).
					Select(v => v.ReshapeAsMatrix(rows, columns).AsIndexable()).ToList()));
