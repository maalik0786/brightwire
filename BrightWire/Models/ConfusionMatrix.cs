...
namespace BrightWire.Models
{
	...
	public class ConfusionMatrix
	{
		...
		Dictionary<string, int> _classificationTable;
			...
			get
			{
				if (_classificationTable == null)
				{
					lock (this)
					{
						if (_classificationTable == null)
						{
							_classificationTable = ClassificationLabels.Select((c, i) => (c, i)).
		...
		public uint GetCount(string expected, string actual)
		{
			...
			var expectedClassification =
				Classifications.FirstOrDefault(c => c.ClassificationIndex == expectedIndex);
			if (expectedClassification != null)
			{
				var actualIndex = ClassificationTable[actual];
				var actualClassification =
					expectedClassification.ActualClassifications.FirstOrDefault(c =>
						c.ClassificationIndex == actualIndex);
