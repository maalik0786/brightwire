...
namespace BrightWire.Models.DataTable
{
	...
	public class DataTableVectorisation
	{
		...
		Dictionary<int, Dictionary<int, string>> _reverseColumnMap;
		Dictionary<int, Dictionary<string, int>> _columnMap;
			...
			get
			{
				if (_columnMap == null)
				{
					lock (this)
					{
						if (_columnMap == null)
						{
							_columnMap = Columns.Where(c => c.Values != null).
								Select(c => (c.ColumnIndex,
									c.Values.ToDictionary(cv => cv.Category, cv => cv.Index))).
			...
			get
			{
				if (_reverseColumnMap == null)
				{
					lock (this)
					{
						if (_reverseColumnMap == null)
						{
							_reverseColumnMap = Columns.Where(c => c.Values != null).
								Select(c => (c.ColumnIndex,
									c.Values.ToDictionary(cv => cv.Index, cv => cv.Category))).
								ToDictionary(d => d.Item1, d => d.Item2);
						}
					}
				}
		...
		{
			if (HasTarget)
				return _Reverse(vector, ClassColumnIndex, targetColumnType, IsTargetBinary,
					IsTargetContinuous);
			return vector;
		}

		object _Reverse(FloatVector vector, int columnIndex, ColumnType columnType, bool isBinary,
			bool isContinuous)
		{
			if (isBinary)
			...
			if (columnType == ColumnType.IndexList)
			{
				...
				{
					Index = vector.Data.Select((v, i) => (v, (uint)i)).
						Where(d => BoundMath.IsNotZero(d.Item1)).Select(d => d.Item2).ToArray()
				};
			}
			...
			if (columnType == ColumnType.WeightedIndexList)
			{
				...
				{
					IndexList = vector.Data.Select((v, i) => (v, (uint)i)).
						Where(d => BoundMath.IsNotZero(d.Item1)).Select(d =>
							new WeightedIndexList.WeightedIndex { Index = d.Item2, Weight = d.Item1 }).ToArray()
				};
			}
