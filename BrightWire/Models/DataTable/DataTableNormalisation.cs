...
namespace BrightWire.Models.DataTable
{
	...
	public class DataTableNormalisation
	{
		...
		Dictionary<int, Column> _columnTable;
		Dictionary<int, Column> ColumnTable
		{
			get
			{
				return _columnTable ??
					(_columnTable = ColumnNormalisation?.ToDictionary(c => c.ColumnIndex, c => c));
			}
		}

		Dictionary<int, VectorColumn> _vectorColumnTable;
		Dictionary<int, VectorColumn> VectorColumnTable
		{
			get
			{
				return _vectorColumnTable ?? (_vectorColumnTable =
					VectorColumnNormalisation?.ToDictionary(c => c.ColumnIndex, c => c));
			}
		}
		...
		object _Normalise(int columnIndex, object obj, bool normalise)
		{
				...
				VectorColumnTable.TryGetValue(columnIndex, out var vectorNorm))
			{
				...
				ret = new FloatVector { Data = normalised };
			}
			else
				ret = obj;
