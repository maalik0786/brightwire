	...
	public class FloatTensor
	{
		...
		public static FloatTensor Create(FloatMatrix[] matrices)
		{
			...
			if (matrices != null)
			{
				...
				Debug.Assert(matrices.All(m =>
					m.RowCount == firstRowCount && m.ColumnCount == firstColumnCount));
			}
#endif
		...
		{
			if (tensor == null || RowCount != tensor.RowCount || ColumnCount != tensor.ColumnCount ||
				Depth != tensor.Depth)
				return false;
