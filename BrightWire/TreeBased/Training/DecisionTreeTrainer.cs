...
namespace BrightWire.TreeBased.Training
{
	...
	internal static class DecisionTreeTrainer
	{
		class Attribute
		{
			...
			public override bool Equals(object obj)
			{
				if (obj is Attribute other)
					return other.ColumnIndex == ColumnIndex && other.Category == Category &&
						Split == other.Split;
				return false;
			}
			...
			{
				if (Category != null)
					return $"{Category} ({ColumnIndex})";
				return $"threshold: {Split} ({ColumnIndex}";
			}
			...
			public IReadOnlyDictionary<string, List<InMemoryRow>> Partition(IEnumerable<InMemoryRow> rows)
			{
				...
				if (Category != null)
				{
					...
					{
						var val = item.GetCategory(ColumnIndex);
				...
				var splitVal = Split ?? 0;
				foreach (var item in rows)
				{
					var val = item.GetValue(ColumnIndex);
		...
		class InMemoryRow
		{
			...
			public InMemoryRow(IRow row, HashSet<int> categorical, HashSet<int> continuous,
		...
		class TableInfo
		{
			...
			public TableInfo(IDataTable table)
			{
				...
				table.ForEach(row =>
					_data.Add(new InMemoryRow(row, _categorical, _continuous, ClassColumnIndex)));
			}
		...
		class Node
		{
			...
			public Node(TableInfo tableInfo, IReadOnlyList<InMemoryRow> data, string matchLabel)
			{
				...
				_classCount = data.GroupBy(d => d.ClassificationLabel).
			...
			public string PredictedClass =>
				_classCount.OrderByDescending(kv => kv.Value).Select(kv => kv.Key).FirstOrDefault();
		...
		public static DecisionTree Train(IDataTable table, Config config = null)
		{
			...
			while (stack.Any())
			{
				...
				{
					var newChildren = item.Partition(node.Data).
						Select(d => new Node(tableInfo, d.Value, d.Key)).ToList();
				...
				var bestSplit = scoreTable.OrderByDescending(kv => kv.Value).Select(kv => kv.Key).
					FirstOrDefault();
				if (bestSplit != null)
				{
			...
			{
				ClassColumnIndex = tableInfo.ClassColumnIndex, Root = root.AsDecisionTreeNode()
			};
		}
