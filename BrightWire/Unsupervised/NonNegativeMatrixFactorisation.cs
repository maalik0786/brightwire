...
namespace BrightWire.Unsupervised
{
	...
	class NonNegativeMatrixFactorisation
	{
		...
		public IReadOnlyList<IReadOnlyList<IVector>> Cluster(IReadOnlyList<IVector> data,
			int numIterations, float errorThreshold = 0.001f)
		{
			if (data.Count == 0)
			...
			var rand = new Random();
			var weights = _lap.CreateMatrix(v.RowCount, _numClusters,
				(x, y) => Convert.ToSingle(rand.NextDouble()));
			var features = _lap.CreateMatrix(_numClusters, v.ColumnCount,
				(x, y) => Convert.ToSingle(rand.NextDouble()));
			...
			var documentClusters = weights.AsIndexable().Rows.
				Select((c, i) => Tuple.Create(i, c.MaximumIndex())).ToList();
			weights.Dispose();
			features.Dispose();
			return documentClusters.GroupBy(d => d.Item2).
				Select(g => g.Select(d => data[d.Item1]).ToArray()).ToList();
		}

		float _DifferenceCost(IMatrix m1, IMatrix m2)
		{
			return m1.AsIndexable().Rows.
				Zip(m2.AsIndexable().Rows, (r1, r2) => _costFunction.Compute(r1.Data, r2.Data)).Average();
		}
	}
}