...
namespace BrightWire.Unsupervised
{
	...
	class KMeans : IDisposable
	{
		...
		List<(int[] DataIndices, IVector Cluster)> _clusters =
			new List<(int[] DataIndices, IVector Cluster)>();
		readonly IReadOnlyList<IVector> _data;

		public KMeans(ILinearAlgebraProvider lap, int k, IReadOnlyList<IVector> data,
			DistanceMetric distanceMetric = DistanceMetric.Euclidean, int? randomSeed = null)
		{
			_data = data;
			...
			AddCluster(rand.Next(0, data.Count),
				(index, distance) => distanceTable[index] = new List<float> { distance });
		...
		public bool Cluster()
		{
			...
			var clusters = closest.Select((ci, i) => (ci, i)).GroupBy(d => d.Item1).
				Select(c => c.Select(d => d.Item2).ToArray()).ToList();
			...
			for (var i = 0; i < _clusters.Count; i++)
			{
				...
				if (i < clusters.Count)
				{
					...
					if (oldIndices.Length == newIndices.Length &&
						new HashSet<int>(oldIndices).SetEquals(newIndices))
		...
		public IReadOnlyList<IReadOnlyList<IVector>> Clusters =>
			_clusters.Select(c => c.DataIndices.Select(i => _data[i]).ToList()).ToList();
