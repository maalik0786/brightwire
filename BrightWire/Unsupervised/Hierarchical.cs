...
namespace BrightWire.Unsupervised
{
	...
	class Hierarchical : IDisposable
	{
		...
		{
			readonly Dictionary<(Centroid, Centroid), float> _distance =
				new Dictionary<(Centroid, Centroid), float>();
		...
		public Hierarchical(int k, IReadOnlyList<IVector> data,
		...
		{
			get { return _centroid.Select(c => c.Data).ToList(); }
		}
	}
}