...
namespace BrightWire.ExecutionGraph.Engine.Helper
{
	...
	{
		readonly ConcurrentQueue<IGraphOperation> _operationList =
			new ConcurrentQueue<IGraphOperation>();
		readonly ConcurrentDictionary<string, IMatrix> _memory =
			new ConcurrentDictionary<string, IMatrix>();
		readonly ConcurrentDictionary<IMiniBatchSequence, Action<IContext>> _continuationTable =
			new ConcurrentDictionary<IMiniBatchSequence, Action<IContext>>();
		...
		public void RegisterContinuation(IMiniBatchSequence sequence, Action<IContext> callback) =>
			_continuationTable[sequence] = callback;
		...
		{
			if (_continuationTable.TryRemove(context.BatchSequence, out Action<IContext> callback))
				callback(context);
		}
