...
namespace BrightWire.ExecutionGraph.Engine.Helper
{
	...
	class SingleRowDataSource : IDataSource
	{
		...
		{
			public Sequence(IReadOnlyList<IGraphData> data, IMiniBatch miniBatch,
		...
		public SingleRowDataSource(float[] data, bool isSequential, MiniBatchSequenceType sequenceType,
			int sequenceIndex)
		{
			_data = data;
		...
		public IMiniBatch Get(IExecutionContext executionContext, IReadOnlyList<int> rows)
		{
			...
			return new SingleRowMiniBatch(this, new MatrixGraphData(data.ReshapeAsRowMatrix()),
				IsSequential, _sequenceType, _sequenceIndex);
		}
