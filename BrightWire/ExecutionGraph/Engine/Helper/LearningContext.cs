...
namespace BrightWire.ExecutionGraph.Engine.Helper
{
	...
	class LearningContext : ILearningContext
	{
		...
		readonly List<(object Error, Action<object> Updater)> _layerUpdate =
			new List<(object, Action<object>)>();
		readonly Stack<(IGraphData Data, Action<IGraphData> Callback)> _deferredBackpropagation =
			new Stack<(IGraphData, Action<IGraphData>)>();
		readonly Stopwatch _timer = new Stopwatch();
		readonly HashSet<INode> _noUpdateNodeSet = new HashSet<INode>();

		public LearningContext(ILinearAlgebraProvider lap, float learningRate, int batchSize,
			TrainingErrorCalculation trainingErrorCalculation, bool deferUpdates)
		{
