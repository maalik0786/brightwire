...
namespace BrightWire.ExecutionGraph.Engine
{
	...
	{
		readonly
			List<(IMiniBatchSequence Sequence, double? TrainingError, IReadOnlyList<FloatMatrix> Output)>
			_executionResults =
				new List<(IMiniBatchSequence Sequence, double? TrainingError, IReadOnlyList<FloatMatrix>
					Output)>();
		readonly List<IContext> _contextList = new List<IContext>();
		readonly IReadOnlyList<INode> _input;
		readonly bool _isStochastic;
		float? _lastTestError;
		double? _lastTrainingError, _trainingErrorDelta;

		public TrainingEngine(ILinearAlgebraProvider lap, IDataSource dataSource,
			ILearningContext learningContext, INode start) : base(lap)
		{
			...
			if (start == null)
			{
				_input = Enumerable.Range(0, dataSource.InputCount).Select(i => new InputFeeder(i)).
					ToList();
			...
			Action<float> batchCompleteCallback = null)
		{
			...
			using (var executionContext = new ExecutionContext(_lap))
			{
				executionContext.Add(provider.GetMiniBatches(batchSize,
					mb => _Execute(executionContext, mb)));
		...
		public double Train(IExecutionContext executionContext,
			Action<float> batchCompleteCallback = null)
		{
			_lap.PushLayer();
			...
			executionContext.Add(provider.GetMiniBatches(LearningContext.BatchSize,
				batch => _contextList.AddRange(_Train(executionContext, LearningContext, batch))));
			IGraphOperation operation;
			float operationCount = executionContext.RemainingOperationCount;
			float index = 0f;
			while ((operation = executionContext.GetNextOperation()) != null)
		...
		IReadOnlyList<IContext> _Train(IExecutionContext executionContext,
			ILearningContext learningContext, IMiniBatch batch)
		{
			...
			if (batch.IsSequential)
			{
				...
				var contextTable =
					new Lazy<Dictionary<IMiniBatchSequence, TrainingEngineContext>>(() =>
						ret.ToDictionary(c => c.BatchSequence, c => c));
				var didContinue =
					_Continue(batch, executionContext, sequence => contextTable.Value[sequence]);
				if (didContinue)
				{
		...
		void _CompleteSequence(TrainingEngineContext context)
		{
			...
			_executionResults.Add((context.BatchSequence, context.TrainingError,
				output.Any() ? output.Select(o => o.GetMatrix().Data).ToArray()
					: new[] { context.Data.GetMatrix().Data }));
		}

		TrainingEngineContext _Train(IExecutionContext executionContext,
			ILearningContext learningContext, IMiniBatchSequence sequence)
		{
		...
		public bool Test(IDataSource testDataSource, IErrorMetric errorMetric, int batchSize = 128,
			Action<float> batchCompleteCallback = null, Action<float, double, bool, bool> values = null)
		{
			var testError = Execute(testDataSource, batchSize, batchCompleteCallback).
				Where(b => b.Target != null).Average(o => o.CalculateError(errorMetric));
			...
			{
				trainingScore = _Write(" training", Convert.ToSingle(_lastTrainingError ?? 0),
					LearningContext.TrainingErrorCalculation == TrainingErrorCalculation.TrainingData &&
					isPercentage);
			...
			var msg =
				$"\rEpoch {LearningContext.CurrentEpoch} -{trainingScore} time: {LearningContext.EpochSeconds:N2}s; {testScore}";
			if (flag)
				msg += "!!";
