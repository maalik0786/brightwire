...
namespace BrightWire.ExecutionGraph.Engine
{
	...
	{
		readonly List<(ExecutionEngineContext Context, IReadOnlyList<IMatrix> Data)> _executionResults =
			new List<(ExecutionEngineContext, IReadOnlyList<IMatrix>)>();
		...
		protected override IReadOnlyList<ExecutionResult> _GetResults()
		{
			...
			foreach (var item in _executionResults)
			{
				...
				{
					ret.Add(new ExecutionResult(item.Context.BatchSequence,
						output.AsIndexable().Rows.Select(r => r.Data).ToList(), outputIndex));
					++outputIndex;
				}
		...
		protected override void _Execute(IExecutionContext executionContext, IMiniBatch batch)
		{
			...
			foreach (var item in ret)
			{
				...
				_executionResults.Add((item,
					output.Any() ? output.Select(o => o.GetMatrix()).ToArray()
