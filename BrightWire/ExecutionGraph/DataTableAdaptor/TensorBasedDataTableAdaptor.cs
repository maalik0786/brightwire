...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	class TensorBasedDataTableAdaptor : RowBasedDataTableAdaptorBase, IVolumeDataSource
	{
		...
		public TensorBasedDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable) : base(lap,
			dataTable)
		{
		...
		TensorBasedDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable, int inputSize,
			int outputSize, int rows, int columns, int depth) : base(lap, dataTable)
		{
		...
		{
			return new TensorBasedDataTableAdaptor(_lap, dataTable, _inputSize, _outputSize, Height,
				Width, Depth);
		}
		...
		{
			var data = _GetRows(rows).Select(r => (
				_dataColumnIndex.Select(i => ((FloatTensor)r.Data[i]).GetAsRaw()).ToList(),
				((FloatVector)r.Data[_dataTargetIndex]).Data)).ToList();
			...
			var output = OutputSize > 0
				? _lap.CreateMatrix(data.Count, OutputSize, (x, y) => data[x].Item2[y]) : null;
