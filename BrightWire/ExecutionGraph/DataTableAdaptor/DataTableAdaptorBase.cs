...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	public abstract class DataTableAdaptorBase<T> : IDataSource
	{
		...
		protected DataTableAdaptorBase(ILinearAlgebraProvider lap, IDataTable dataTable)
		{
			...
			_dataColumnIndex = Enumerable.Range(0, dataTable.ColumnCount).
				Where(ci => ci != _dataTargetIndex).ToArray();
		}
		...
		protected IMiniBatch _GetMiniBatch(IReadOnlyList<int> rows,
			IReadOnlyList<(float[][], float[])> data)
		{
			var inputList = new List<IGraphData>();
			for (int i = 0, len = data.First().Item1.Length; i < len; i++)
			{
				var i1 = i;
				inputList.Add(new MatrixGraphData(_lap.CreateMatrix(data.Count, InputSize,
					(x, y) => data[x].Item1[i1][y])));
			}
		...
		protected IMiniBatch _GetSequentialMiniBatch(IReadOnlyList<int> rows,
			IReadOnlyList<(FloatMatrix Input, FloatMatrix Output)> data)
		{
