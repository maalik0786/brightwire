...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	/// </summary>
	class IndexListDataTableAdaptor : DataTableAdaptorBase<(List<IndexList>, FloatVector)>,
		IIndexListEncoder
	{
		readonly int _inputSize;
		readonly IDataTableVectoriser _vectoriser;

		public IndexListDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable,
			IDataTableVectoriser vectoriser) : base(lap, dataTable)
		{
			...
			// load the data
			dataTable.ForEach(row => _data.Add((_dataColumnIndex.Select(row.GetField<IndexList>).ToList(),
