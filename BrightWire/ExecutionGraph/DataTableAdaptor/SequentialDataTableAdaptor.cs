...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	class SequentialDataTableAdaptor : RowBasedDataTableAdaptorBase
	{
		...
		public SequentialDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable) : base(lap,
			dataTable)
		{
		...
		{
			var data = _GetRows(rows).Select(r => ((FloatMatrix)r.Data[0], (FloatMatrix)r.Data[1])).
				ToList();
