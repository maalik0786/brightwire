...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	{
		public VectorBasedDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable) : base(lap,
			dataTable)
		{
		...
		public override IMiniBatch Get(IExecutionContext executionContext, IReadOnlyList<int> rows)
		{
			var data = _GetRows(rows).Select(r => ((
				_dataColumnIndex.Select(i => ((FloatVector)r.Data[i]).Data).ToArray(),
