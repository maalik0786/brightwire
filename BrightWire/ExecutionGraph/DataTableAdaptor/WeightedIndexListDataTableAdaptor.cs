...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	/// </summary>
	class WeightedIndexListDataTableAdaptor :
		DataTableAdaptorBase<(List<WeightedIndexList>, FloatVector)>, IWeightedIndexListEncoder
	{
		readonly int _inputSize;
		readonly IDataTableVectoriser _vectoriser;

		public WeightedIndexListDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable,
			IDataTableVectoriser vectoriser) : base(lap, dataTable)
		{
			...
			// load the data
			dataTable.ForEach(row => _data.Add((
				_dataColumnIndex.Select(row.GetField<WeightedIndexList>).ToList(),
				_vectoriser.GetOutput(row))));
		}
		...
		{
			var data = _GetRows(rows).Select(r => (r.Item1.Select(Encode).ToArray(), r.Item2.Data)).
				ToList();
