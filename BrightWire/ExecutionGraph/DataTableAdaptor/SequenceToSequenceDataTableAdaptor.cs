...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	class SequenceToSequenceDataTableAdaptor : AdaptiveDataTableAdaptorBase
	{
		...
		public SequenceToSequenceDataTableAdaptor(ILinearAlgebraProvider lap,
			ILearningContext learningContext, GraphFactory factory, IDataTable dataTable,
			Action<WireBuilder> dataConversionBuilder) : base(lap, learningContext, dataTable)
		{
		...
		private SequenceToSequenceDataTableAdaptor(ILinearAlgebraProvider lap,
			ILearningContext learningContext, IDataTable dataTable, INode input, int inputSize,
			int outputSize) : base(lap, learningContext, dataTable)
		{
			_Initialise(dataTable);
			_input = input;
		...
		{
			return new SequenceToSequenceDataTableAdaptor(_lap, _learningContext, dataTable, _input,
				_inputSize, _outputSize);
		}
		...
		{
			return _rowDepth.Select((r, i) => (r, i)).GroupBy(t => t.Item1).
				Select(g => g.Select(d => d.Item2).ToList()).ToList();
		}

		(IMatrix, IReadOnlyList<IRow>) _Encode(IExecutionContext executionContext,
		...
		public override IMiniBatch Get(IExecutionContext executionContext, IReadOnlyList<int> rows)
		{
			...
			foreach (var item in outputData.OrderBy(kv => kv.Key))
			{
				...
				var type = (item.Key == 0) ? MiniBatchSequenceType.SequenceStart :
					item.Key == (outputData.Count - 1) ? MiniBatchSequenceType.SequenceEnd :
					MiniBatchSequenceType.Standard;
				var inputList = new List<IGraphData> { new MatrixGraphData(curr) };
		...
		public override void OnBatchProcessed(IContext context)
		{
			...
			if (context.IsTraining && batch.Type == MiniBatchSequenceType.SequenceStart)
			{
				context.LearningContext.DeferBackpropagation(null,
					signal => { _learningContext.BackpropagateThroughTime(signal); });
			}
		}
	}
}