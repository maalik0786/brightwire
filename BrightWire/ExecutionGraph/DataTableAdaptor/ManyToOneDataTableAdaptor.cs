...
namespace BrightWire.ExecutionGraph.DataTableAdaptor
{
	...
	class ManyToOneDataTableAdaptor : RowBasedDataTableAdaptorBase
	{
		...
		public ManyToOneDataTableAdaptor(ILinearAlgebraProvider lap, IDataTable dataTable) : base(lap,
			dataTable)
		{
		...
		public override IMiniBatch Get(IExecutionContext executionContext, IReadOnlyList<int> rows)
		{
			var data = _GetRows(rows).Select(r =>
				((FloatMatrix)r.Data[_dataColumnIndex[0]], (FloatVector)r.Data[_dataTargetIndex])).ToList();
			...
			foreach (var item in inputData.OrderBy(kv => kv.Key))
			{
				...
				miniBatch.Add(type, inputList,
					type == MiniBatchSequenceType.SequenceEnd ? new MatrixGraphData(outputVector) : null);
			}
