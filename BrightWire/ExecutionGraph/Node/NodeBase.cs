...
namespace BrightWire.ExecutionGraph.Node
{
	...
	public abstract class NodeBase : INode
	{
		...
		protected void _AddNextGraphAction(IContext context, IGraphData data,
			Func<IBackpropagation> backProp)
		{
		...
		/// <returns></returns>
		public virtual Models.ExecutionGraph.Node SerialiseTo(HashSet<INode> existing,
			List<Models.ExecutionGraph.Node> connectedTo, HashSet<Models.ExecutionGraph.Wire> wireList)
		{
			var info = _GetInfo();
			...
			if (connectedTo != null && wireList != null)
			{
				foreach (var wire in Output)
				{
					...
					{
						FromId = Id, InputChannel = wire.Channel, ToId = sendTo.Id
					});
		...
		void ICanInitialiseNode.Initialise(GraphFactory factory, string id, string name,
