...
namespace BrightWire.ExecutionGraph.Node.Gate
{
	class ReverseTemporalJoin : MultiGateBase
	{
		...
		Dictionary<int, (IMatrix Data, int ReversedSize, INode ForwardParent)> _input =
			new Dictionary<int, (IMatrix Data, int ReversedSize, INode ForwardParent)>();
		Dictionary<int, (IMatrix Data, INode ReverseParent)> _reverseInput =
			new Dictionary<int, (IMatrix Data, INode ReverseParent)>();

		Dictionary<int, (INode, IGraphData)> _reverseBackpropagation =
			new Dictionary<int, (INode, IGraphData)>();
		...
		void _Continue(IContext context)
		{
			...
			context.AddForward(
				new TrainingAction(this, new MatrixGraphData(next),
