...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	class GatedRecurrentUnit : NodeBase, IHaveMemoryNode
	{
		...
		void _Create(GraphFactory graph, int inputSize, float[] memory, string memoryId)
		{
			...
			var Rt = graph.Add(Wr, Ur).AddBackwardAction(new ConstrainSignal()).
				Add(graph.SigmoidActivation("Rt")).LastNode;
			var Zt = graph.Add(Wz, Uz).AddBackwardAction(new ConstrainSignal()).
				Add(graph.SigmoidActivation("Zt")).LastNode;
			...
			var h1 = graph.Add(Wh, Uh).AddBackwardAction(new ConstrainSignal()).
				Add(graph.TanhActivation());

			// h2 = h1x(1-Zt)
			var h2 = graph.Multiply(h1,
				graph.Connect(hiddenLayerSize, Zt).Add(graph.GraphOperation.OneMinusInput()));

			// h = h1xh2
			var previous = graph.Multiply(hiddenLayerSize, Zt, _memory);
			_output = graph.Add(h2, previous).AddForwardAction(_memory.SetMemoryAction)
