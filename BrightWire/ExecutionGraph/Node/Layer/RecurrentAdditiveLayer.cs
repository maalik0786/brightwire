...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	class RecurrentAdditiveLayer : NodeBase, IHaveMemoryNode
	{
		...
		public RecurrentAdditiveLayer(GraphFactory graph, int inputSize, float[] memory,
		...
		void _Create(GraphFactory graph, int inputSize, float[] memory, string memoryId)
		{
			...
			_output = graph.
				Add(graph.Multiply(hiddenLayerSize, Wx, It.LastNode),
					graph.Multiply(hiddenLayerSize, _memory, Ft.LastNode)).
