...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	internal class SimpleRecurrent : NodeBase, IHaveMemoryNode
	{
		...
		INode _input, _output, _activation;
		OneToMany _start;
		int _inputSize;

		public SimpleRecurrent(GraphFactory graph, int inputSize, float[] memory, INode activation,
			...
			string memoryId)
		{
			...
			_output = graph.Add(inputChannel, memoryChannel).AddBackwardAction(new ConstrainSignal()).
				Add(activation).AddForwardAction(_memory.SetMemoryAction)
