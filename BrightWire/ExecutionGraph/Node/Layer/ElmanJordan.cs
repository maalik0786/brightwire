...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	{
		IReadOnlyDictionary<INode, IGraphData> _lastBackpropagation;
		MemoryFeeder _memory;
		INode _input, _output, _activation, _activation2;
		OneToMany _start;
		int _inputSize;
		bool _isElman;

		public ElmanJordan(GraphFactory graph, bool isElman, int inputSize, float[] memory,
			INode activation, INode activation2, string name = null) : base(name)
		{
			_Create(graph, isElman, inputSize, memory, activation, activation2, null);
		}

		void _Create(GraphFactory graph, bool isElman, int inputSize, float[] memory, INode activation,
			INode activation2, string memoryName)
		{
			_isElman = isElman;
			...
			var memoryChannel =
				graph.Connect(hiddenLayerSize, _memory).AddFeedForward(hiddenLayerSize, "Uh");
			var h = graph.Add(inputChannel, memoryChannel).AddBackwardAction(new ConstrainSignal()).
				Add(activation);
			if (isElman)
				h = h.AddForwardAction(_memory.SetMemoryAction);
			h = h.AddFeedForward(hiddenLayerSize, "Wy").AddBackwardAction(new ConstrainSignal()).
				Add(activation2);
			if (!isElman)
