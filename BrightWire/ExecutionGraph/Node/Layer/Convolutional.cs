...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	class Convolutional : NodeBase
	{
		class Backpropagation : SingleBackpropagationBase<Convolutional>
		{
			...
			public Backpropagation(Convolutional source, I3DTensor im2Col, int inputWidth,
				int inputHeight, int inputDepth, int inputCount, int newWidth, int newHeight) : base(source)
			{
				_im2Col = im2Col;
				_newWidth = newWidth;
			...
			{
				var tensor = errorSignal.GetMatrix().
					ReshapeAs4DTensor(_newHeight, _newWidth, _source._filter.ColumnCount);
				...
				using (var update = _im2Col.TransposeThisAndMultiply(tensor))
				{
					...
					context.LearningContext.StoreUpdate(_source, weightUpdate,
						err => _source.Update(err, context.LearningContext));
					context.LearningContext.StoreUpdate(_source, biasUpdate,
						bu => _UpdateBias(bu, context.LearningContext));
				}
				...
				if (_source._shouldBackpropagate)
				{
					...
					var reverseIm2Col = tensor.ReverseIm2Col(filters, outputRows, outputColumns, outputDepth,
					...
					return new Tensor4DGraphData(delta.ReshapeAsMatrix(), _inputHeight, _inputWidth,
						inputDepth);
				}
		...
		public Convolutional(bool shouldBackpropagate, IWeightInitialisation weightInitialisation,
			Func<IMatrix, IGradientDescentOptimisation> updater, int inputDepth, int filterCount,
			int padding, int filterWidth, int filterHeight, int xStride, int yStride,
			string name = null) : base(name)
		{
			...
			_filter = weightInitialisation.CreateWeight(_filterWidth * _filterHeight * _inputDepth,
				filterCount);
