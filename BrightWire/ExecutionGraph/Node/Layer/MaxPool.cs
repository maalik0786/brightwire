...
namespace BrightWire.ExecutionGraph.Node.Layer
{
	...
	class MaxPool : NodeBase
	{
		class Backpropagation : SingleBackpropagationBase<MaxPool>
		{
			...
			readonly int _inputColumns, _inputRows, _outputColumns, _outputRows, _depth, _filterWidth,
				_filterHeight, _xStride, _yStride;

			public Backpropagation(MaxPool source, I4DTensor indices, int inputColumns, int inputRows,
				int outputColumns, int outputRows, int depth, int filterWidth, int filterHeight,
				int xStride, int yStride) : base(source)
			{
				_indices = indices;
				...
				IContext context, IReadOnlyList<INode> parents)
			{
				...
				var output = tensor.ReverseMaxPool(_indices, _inputRows, _inputColumns, _filterWidth,
					_filterHeight, _xStride, _yStride);
				...
				//#endif
				return new Tensor4DGraphData(output.ReshapeAsMatrix(), output.RowCount, output.ColumnCount,
					output.Depth);
			}
		}
		...
		public MaxPool(int filterWidth, int filterHeight, int xStride, int yStride,
		...
		public override void ExecuteForward(IContext context)
		{
			...
			_AddNextGraphAction(context, graphData,
				() => new Backpropagation(this, index, tensor.ColumnCount, tensor.RowCount,
					output.ColumnCount, output.RowCount, output.Depth, _filterWidth, _filterHeight, _xStride,
					_yStride));
		}
