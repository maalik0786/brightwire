...
namespace BrightWire.ExecutionGraph.Node
{
	...
	public abstract class BackpropagationBase<T> : IBackpropagation where T : INode
	{
		/// <summary>
		...
		protected BackpropagationBase(T source)
		{
			_source = source;
		}
		...
		public void Backward(INode fromNode, IGraphData errorSignal, IContext context,
		...
		/// <param name="context">Graph context</param>
		/// <param name="parents">Parents of the current node</param>
		public abstract void _Backward(INode fromNode, IGraphData errorSignal, IContext context,
			IReadOnlyList<INode> parents);
		...
		/// <param name="context">Graph context</param>
		/// <param name="parents">Parents of the current node</param>
		protected void _SendErrorTo(IGraphData errorSignal, IContext context,
			IReadOnlyList<INode> parents)
		{
			if (parents?.Any() == true)
			{
