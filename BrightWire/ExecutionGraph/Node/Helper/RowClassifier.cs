...
namespace BrightWire.ExecutionGraph.Node.Helper
{
	...
	class RowClassifier : NodeBase
	{
		...
		public override void ExecuteForward(IContext context)
		{
			...
			{
				var value = _classifier.Classify(row).Select(c => (_targetLabel[c.Label], c.Weight)).
					ToDictionary(d => d.Item1, d => d.Item2);
				resultList.Add(value);
			}

			var output = _lap.CreateMatrix(resultList.Count, _targetLabel.Count,
				(i, j) => resultList[i].TryGetValue(j, out float temp) ? temp : 0f);
