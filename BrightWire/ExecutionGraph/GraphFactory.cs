...
namespace BrightWire.ExecutionGraph
{
	...
	public class GraphFactory
	{
		...
		public IPropertySet CurrentPropertySet =>
			_propertySetStack.Any() ? _propertySetStack.Peek() : _defaultPropertySet;
		...
		public IGradientDescentOptimisation CreateWeightUpdater(IMatrix weight)
		{
			...
			if (createTemplateGradientDescent != null)
				ret = createTemplateGradientDescent.Create(ret ?? SimpleGradientDescent, weight,
					propertySet);
		...
		/// <returns></returns>
		public IDataSource CreateDataSource(IDataTable dataTable,
			IDataTableVectoriser vectoriser = null)
		{
			var columns = dataTable.Columns;
			var dataColumnTypes = columns.Where((c, i) => i != dataTable.TargetColumnIndex).
			...
			if (firstDataColumnType != ColumnType.Null &&
				dataColumnTypes.All(ct => ct == firstDataColumnType))
			{
				// many to many
				...
				// one to one
				if (firstDataColumnType == ColumnType.Vector && targetColumnType == ColumnType.Vector)
					return new VectorBasedDataTableAdaptor(LinearAlgebraProvider, dataTable);

				// one to many
				if (firstDataColumnType == ColumnType.Vector && targetColumnType == ColumnType.Matrix)
					return new OneToManyDataTableAdaptor(LinearAlgebraProvider, dataTable);

				// many to one
				if (firstDataColumnType == ColumnType.Matrix && targetColumnType == ColumnType.Vector)
					return new ManyToOneDataTableAdaptor(LinearAlgebraProvider, dataTable);

				// volume classification
				if (firstDataColumnType == ColumnType.Tensor && targetColumnType == ColumnType.Vector)
					return new TensorBasedDataTableAdaptor(LinearAlgebraProvider, dataTable);

				// index list
				if (firstDataColumnType == ColumnType.IndexList)
					return new IndexListDataTableAdaptor(LinearAlgebraProvider, dataTable, vectoriser);

				// weighted index list
				if (firstDataColumnType == ColumnType.WeightedIndexList)
					return new WeightedIndexListDataTableAdaptor(LinearAlgebraProvider, dataTable,
						vectoriser);
			}

			// default adapator
			return new DefaultDataTableAdaptor(LinearAlgebraProvider, dataTable, vectoriser);
		}
		...
		/// <returns></returns>
		public IDataSource CreateDataSource(IDataTable dataTable, ILearningContext learningContext,
			...
			if (columns.Count == 2)
			{
				...
				if (column1 == ColumnType.Matrix && column2 == ColumnType.Matrix)
					return new SequenceToSequenceDataTableAdaptor(LinearAlgebraProvider, learningContext,
						this, dataTable, dataConversionBuilder);
			}
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateDropConnect(float dropoutPercentage, int inputSize, int outputSize,
			string name = null)
		{
			// create weights and bias
			var weightInit = _GetWeightInitialisation();
			...
			// get the gradient descent optimisations
			var optimisation = CreateWeightUpdater(weight);
			return new DropConnect(dropoutPercentage, inputSize, outputSize, bias, weight,
				LinearAlgebraProvider.IsStochastic, optimisation, name);
		}
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateConvolutional(int inputDepth, int filterCount, int padding, int filterWidth,
			int filterHeight, int xStride, int yStride, bool shouldBackpropagate = true,
			string name = null)
		{
			var weightInit = _GetWeightInitialisation();
			return new Convolutional(shouldBackpropagate, weightInit, CreateWeightUpdater, inputDepth,
				filterCount, padding, filterWidth, filterHeight, xStride, yStride, name);
		}
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateMaxPool(int filterWidth, int filterHeight, int xStride, int yStride,
			string name = null)
		{
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateSimpleRecurrent(int inputSize, float[] memory, INode activation,
			string name = null)
		{
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateElman(int inputSize, float[] memory, INode activation, INode activation2,
			string name = null)
		{
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public INode CreateJordan(int inputSize, float[] memory, INode activation, INode activation2,
			string name = null)
		{
		...
		/// <param name="name">Optional name to give the node</param>
		/// <returns></returns>
		public WireBuilder ReverseTemporalJoin(WireBuilder forwardInput, WireBuilder backwardInput,
			string name = null)
		{
		...
		public IWeightInitialisation ConstantWeightInitialisation(float biasValue = 0f,
			float weightValue = 1f) =>
			new Constant(LinearAlgebraProvider, biasValue, weightValue);
		...
		public IWeightInitialisation GaussianWeightInitialisation(bool zeroBias = true,
			float stdDev = 0.1f,
		...
		public IWeightInitialisation IdentityWeightInitialisation(float identityValue = 1f) =>
			new Identity(LinearAlgebraProvider, identityValue);

		/// <summary>
		/// Creates a xavier weight initialiser
		/// </summary>
		/// <param name="parameter">Xavier parameter</param>
		public IWeightInitialisation XavierWeightInitialisation(float parameter = 6) =>
			new Xavier(LinearAlgebraProvider, parameter);
		...
		public ICreateTemplateBasedGradientDescent Adam(float decay = 0.9f, float decay2 = 0.99f) =>
			new AdamDescriptor(decay, decay2);
		...
		public ICreateTemplateBasedGradientDescent Momentum(float momentum = 0.9f) =>
			new MomentumDescriptor(momentum);
		...
		/// <returns></returns>
		public ICreateTemplateBasedGradientDescent NesterovMomentum(float momentum = 0.9f) =>
			new NesterovMomentumDescriptor(momentum);
		...
		/// <returns></returns>
		public ICreateTemplateBasedGradientDescent RmsProp(float decay = 0.9f) =>
			new RmsPropDescriptor(decay);

		/// <summary>
		/// Uses vanilla stochastic gradient descent
		/// </summary>
		public IGradientDescentOptimisation SimpleGradientDescent { get; } =
		...
		public class GradientDescentProvider
		{
			...
			/// </summary>
			public ICreateTemplateBasedGradientDescent NesterovMomentum { get; } =
				new NesterovMomentumDescriptor();
		...
		public class ErrorMetricProvider
		{
			...
			/// </summary>
			public IErrorMetric BinaryClassification { get; } = new BinaryClassification();

			/// <summary>
			/// Cross entropy error metric
			/// </summary>
			public IErrorMetric CrossEntropy { get; } = new CrossEntropy();

			/// <summary>
			/// One hot encoding error metric
			/// </summary>
			public IErrorMetric OneHotEncoding { get; } = new OneHotEncoding();

			/// <summary>
			/// Quadratic error metric
			/// </summary>
			public IErrorMetric Quadratic { get; } = new Quadratic();
		}
