...
namespace BrightWire.Linear.Training
{
	...
	internal class LogisticRegressionTrainer : ILogisticRegressionTrainer
	{
		...
		public LogisticRegression GradientDescent(int iterations, float learningRate,
			float lambda = 0.1f, Func<float, bool> costCallback = null)
		{
			...
			var ret = new LogisticRegression { Theta = theta.Data };
			theta.Dispose();
			return ret;
		}
		...
		public float ComputeCost(IVector th, float lambda)
		{
			...
			if (BoundMath.IsNotZero(lambda))
				ret += th.AsIndexable().Values.Skip(1).Select(v => v * v).Sum() * lambda /
					(2 * _feature.RowCount);
			return ret;
		}
