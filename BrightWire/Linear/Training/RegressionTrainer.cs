...
namespace BrightWire.Linear.Training
{
	...
	class RegressionTrainer : ILinearRegressionTrainer
	{
		...
		public RegressionTrainer(ILinearAlgebraProvider lap, IDataTable table)
		{
			...
			var data =
				table.GetNumericColumns(Enumerable.Range(0, numCols).Where(c => c != classColumnIndex));
		...
		public LinearRegression GradientDescent(int iterations, float learningRate, float lambda = 0.1f,
			Func<float, bool> costCallback = null)
		{
			...
			using (var regularisationVector =
				_lap.CreateVector(theta.Count, i => i == 0 ? 1f : regularisation))
			{
			...
			var ret = new LinearRegression { Theta = theta.Data };
			theta.Dispose();
			return ret;
		}
