...
namespace BrightWire
{
	...
	public interface IDataTable : IHaveColumns
	{
		...
		(IDataTable Training, IDataTable Test) Split(int? randomSeed = null,
			double trainingPercentage = 0.8, bool shuffle = true, Stream output1 = null,
			Stream output2 = null);
		...
		/// <returns></returns>
		IEnumerable<(IDataTable Training, IDataTable Validation)> Fold(int k, int? randomSeed = null,
			bool shuffle = true);
		...
		IDataTable ChangeColumnTypes(Dictionary<int, IConvertToType> columnConversion,
	...
	public interface IDataTableBuilder : IDisposable, IRowProcessor
	{
		...
		/// <param name="isTarget">True if the column is a classification target</param>
		/// <returns></returns>
		IColumn AddTensorColumn(int rows, int columns, int depth, string name = "",
			bool isTarget = false);
