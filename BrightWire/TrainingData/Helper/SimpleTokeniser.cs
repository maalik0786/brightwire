	...
	public static class SimpleTokeniser
	{
		...
		public static IEnumerable<string> Tokenise(string text)
		{
			...
			foreach (var ch in text)
			{
				...
				if ((isWord && parseState != ParseState.InWord) ||
					(isNumber && parseState != ParseState.InNumber) ||
					(isSpace && parseState != ParseState.None))
				{
					if (sb.Length > 0)
					{
						yield return sb.ToString();
					...
					parseState = isWord ? ParseState.InWord :
						isNumber ? ParseState.InNumber : ParseState.None;
				}
