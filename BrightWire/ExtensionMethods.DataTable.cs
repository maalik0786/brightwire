...
namespace BrightWire
{
	public static partial class ExtensionMethods
	{
		...
		public static IReadOnlyList<(string Label, WeightedIndexList Data)> ConvertToWeightedIndexList(
			this IReadOnlyList<(string Label, IndexList Data)> data, bool groupByClassification)
		{
			if (groupByClassification)
			{
				return data.GroupBy(c => c.Label).Select(g => (g.Key,
					new WeightedIndexList
					{
						IndexList = g.SelectMany(d => d.Data.Index).GroupBy(d => d).Select(g2 =>
								new WeightedIndexList.WeightedIndex { Index = g2.Key, Weight = g2.Count() }).
							ToArray()
					})).ToArray();
			}

			return data.Select(d => (d.Label,
				new WeightedIndexList
				{
					IndexList = d.Data.Index.GroupBy(i => i).Select(g2 =>
						new WeightedIndexList.WeightedIndex { Index = g2.Key, Weight = g2.Count() }).ToArray()
				})).ToArray();
		}
		...
		/// </summary>
		/// <param name="data"></param>
		public static IDataTable ConvertToTable(
			this IReadOnlyList<(string Label, WeightedIndexList Data)> data)
		{
			var builder = BrightWireProvider.CreateDataTableBuilder();
		...
		/// <returns></returns>
		public static IDataTable ConvertToTable(
			this IReadOnlyList<(string Label, FloatVector Data)> data, bool preserveVectors)
		{
			var builder = BrightWireProvider.CreateDataTableBuilder();
			if (preserveVectors)
			{
		...
		/// </summary>
		/// <param name="data"></param>
		public static IReadOnlyList<(string Classification, FloatVector Data)> Vectorise(
			this IReadOnlyList<(string Label, WeightedIndexList Data)> data)
		{
			...
			FloatVector _Create(WeightedIndexList weightedIndexList)
			{
				...
				return new FloatVector { Data = ret };
			}
		...
		/// <returns></returns>
		public static IReadOnlyList<(string Label, string Classification, float Score)> Classify(
			this IReadOnlyList<(string Label, IndexList Data)> data, IIndexListClassifier classifier)
		{
		...
		/// <param name="data"></param>
		/// <returns></returns>
		public static IReadOnlyList<(string Label, WeightedIndexList Data)> Normalise(
			this IReadOnlyList<(string Label, WeightedIndexList Data)> data)
		{
			var maxWeight = data.GetMaxWeight();
			return data.Select(r => (r.Label,
				new WeightedIndexList
				{
					IndexList = r.Data.IndexList.Select(wi =>
						new WeightedIndexList.WeightedIndex
						{
							Index = wi.Index, Weight = wi.Weight / maxWeight
						}).ToArray()
				})).ToList();
		}
		...
		public static IReadOnlyList<(string Label, WeightedIndexList Data)> TFIDF(
			this IReadOnlyList<(string Label, WeightedIndexList Data)> data)
		{
			...
			foreach (var classification in data)
			{
				...
				foreach (var item in classification.Data.IndexList)
				{
					...
					{
						Index = index, Weight = Convert.ToSingle(score)
					});
				}
		...
		public static object ReverseNormaliseOutput(this DataTableNormalisation model,
			int originalColumnIndex, object valueToConvert)
		{
		...
		/// <returns></returns>
		public static object ReverseVectoriseOutput(this DataTableVectorisation model,
			FloatVector vector, ColumnType targetColumnType)
		{
		...
		public static ConfusionMatrix CreateConfusionMatrix(this IDataTable dataTable,
			int actualClassificationColumnIndex, int expectedClassificationColumnIndex)
		{
			...
			return new ConfusionMatrix
			{
				...
				Classifications = classifications.OrderBy(kv => kv.Key).Select(c =>
					new ConfusionMatrix.ExpectedClassification
					{
						ClassificationIndex = c.Key,
						ActualClassifications = c.Value.OrderBy(kv => kv.Key).Select(c2 =>
							new ConfusionMatrix.ActualClassification
							{
								ClassificationIndex = c2.Key, Count = c2.Value
							}).ToArray()
					}).ToArray()
			};
		}

		public static IDataTable ParseCSVThenSaveToDisk(this StreamReader reader, string dataFilePath,
