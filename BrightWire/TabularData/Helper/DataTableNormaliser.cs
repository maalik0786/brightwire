...
namespace BrightWire.TabularData.Helper
{
	...
	class DataTableNormaliser : IRowProcessor
	{
		...
		public DataTableNormaliser(IDataTable dataTable, NormalisationType type, Stream output,
			IEnumerable<int> columnIndices)
		{
			...
			foreach (var columnInfo in columns)
			{
				...
				else if (column.Type == ColumnType.Vector && columnInfo is IDimensionsColumnInfo vector &&
					vector.XDimension.HasValue && vector.XDimension.Value > 0)
				{
			...
			{
				var collectors = vectorColumns.Select(vc =>
					Enumerable.Range(0, vc.Size).Select(i => new NumberCollector(i)).ToList()).ToList();
		...
		DataTableNormalisation.Column _GetColumn(NormalisationType type, INumericColumnInfo numericInfo,
			int columnIndex, ColumnType columnType)
		{
			if (type == NormalisationType.Standard && !numericInfo.StdDev.HasValue)
				return null;
			if (type == NormalisationType.Standard)
				return new DataTableNormalisation.Column(columnIndex, columnType, numericInfo.StdDev ?? 1,
					numericInfo.Mean);
			if (type == NormalisationType.Euclidean)
			...
			if (type == NormalisationType.FeatureScale)
				return new DataTableNormalisation.Column(columnIndex, columnType,
					numericInfo.Max - numericInfo.Min, numericInfo.Min);
