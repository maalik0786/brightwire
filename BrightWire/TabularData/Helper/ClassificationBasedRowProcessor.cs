...
namespace BrightWire.TabularData.Helper
{
	...
	class ClassificationBasedRowProcessor : IRowProcessor
	{
		...
		readonly Dictionary<string, List<IRowProcessor>> _columnProcessor =
			new Dictionary<string, List<IRowProcessor>>();
		readonly Dictionary<string, uint> _classCount = new Dictionary<string, uint>();
		uint _total;

		public ClassificationBasedRowProcessor(
			IEnumerable<Tuple<string, IRowProcessor>> classificationBasedProcessors,
			int classificationColumnIndex)
		{
		...
		public IEnumerable<Tuple<string, IRowProcessor>> All =>
			_columnProcessor.SelectMany(d => d.Value.Select(d2 => Tuple.Create(d.Key, d2)));
