...
namespace BrightWire.TabularData.Helper
{
	...
	class DataTableVectoriser : IDataTableVectoriser
	{
		...
		public DataTableVectoriser(IDataTable table, bool useTargetColumnIndex,
			int maxNumericCategoricalExpansion = 128)
		{
			var classColumnIndex = useTargetColumnIndex ? table.TargetColumnIndex : -1;
			var analysis = table.GetAnalysis();
			_vectorisationModel = new DataTableVectorisation { ClassColumnIndex = classColumnIndex };
			...
			foreach (var columnInfo in analysis.ColumnInfo)
			{
				...
				else if (columnInfo is INumericColumnInfo)
				{
					columnModel.IsContinuous = column.IsContinuous || !columnInfo.NumDistinct.HasValue ||
						columnInfo.NumDistinct.Value > maxNumericCategoricalExpansion;
		...
		{
			return values.Select(s => s.ToString()).OrderBy(s => s).Select((s, i) =>
				new DataTableVectorisation.CategoricalIndex { Category = s, Index = i }).ToArray();
		}
