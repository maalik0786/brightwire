...
using BrightWire.TabularData.Manipulation;
...
namespace BrightWire.TabularData
{
	...
	class DataTable : IDataTable
	{
		class Column : IColumn
		{
			...
			public Column(int index, string name, ColumnType type, int numDistinct, bool? isContinuous,
				bool isTarget)
			{
				Index = index;
				Name = name;
		...
		public DataTable(Stream stream, IReadOnlyList<long> dataIndex, int rowCount, int? blockSize)
		{
			...
			else
			{
				...
				BlockSize = reader.ReadInt32(); // read the block size
			}
		...
		{
			get { return _column.Any(c => !c.IsContinuous && !c.IsTarget); }
		}
		...
		public IReadOnlyList<IRow> GetSlice(int offset, int count)
		{
			...
			{
				var reader = new BinaryReader(_stream, Encoding.UTF8, true);
				_stream.Seek(_index[offset / BlockSize], SeekOrigin.Begin);

				// seek to offset
				for (var i = 0; i < offset % BlockSize; i++)
					_SkipRow(reader);
		...
		{
			var block = rowIndex / BlockSize;
			var offset = rowIndex % BlockSize;
			lock (_mutex)
			{
				var reader = new BinaryReader(_stream, Encoding.UTF8, true);
		...
		public IReadOnlyList<IRow> GetRows(IReadOnlyList<int> rowIndex)
		{
			...
			lock (_mutex)
			{
				...
				foreach (var block in blockMatch.OrderBy(b => b.Key))
				{
					...
					for (int i = block.Key * BlockSize, len = i + BlockSize;
						i < len && _stream.Position < _stream.Length; i++)
					{
						if (match.Contains(i))
						{
							var row = _ReadDataTableRow(reader);
		...
		public (IDataTable Training, IDataTable Test) Split(int? randomSeed = null,
			double trainPercentage = 0.8, bool shuffle = true, Stream output1 = null,
			Stream output2 = null)
		{
		...
		public IEnumerable<(IDataTable Training, IDataTable Validation)> Fold(int k,
			int? randomSeed = null, bool shuffle = true)
		{
			var input = Enumerable.Range(0, RowCount);
			if (shuffle)
				input = input.Shuffle(randomSeed);
		...
		{
			var columnTable =
				(columns ?? Enumerable.Range(0, ColumnCount)).ToDictionary(i => i,
					i => new float[RowCount]);
			_Iterate((row, i) =>
			{
		...
		public IDataTable Normalise(NormalisationType normalisationType, Stream output = null,
			IEnumerable<int> columnIndices = null)
		{
		...
		public IReadOnlyList<(IDataTable Table, string Classification)> ConvertToBinaryClassification()
		{
			...
			return analysis[TargetColumnIndex].DistinctValues.Select(val => val.ToString()).Select(cls =>
				(Project(r =>
				{
		...
		public IDataTable ChangeColumnTypes(Dictionary<int, IConvertToType> columnConversion,
			bool removeInvalidRows = false)
		{
			return Project(row =>
			{
				var data = row.Data;
				...
				var convertedRow = Enumerable.Range(0, ColumnCount).Select(i =>
					columnConversion.TryGetValue(i, out var converter)
						? Validate(converter.ConvertValue(data[i])) : data[i]).ToList();
				return skipRow ? null : convertedRow;
			});
		}

		public IDataTable Summarise(int newRowCount, Stream output = null,
			ISummariseRows summariser = null)
		{
			if (summariser == null)
