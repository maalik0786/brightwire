...
namespace BrightWire.TabularData.Manipulation
{
	class AverageSummariser : ISummariseRows
	{
		...
		class FrequencyBasedSummariser<T> : SummariserBase
		{
			...
			public override object Result =>
				_frequencyTable.OrderByDescending(kv => kv.Value).Select(kv => kv.Key).FirstOrDefault();
		}
		...
		class NumericSummariser : SummariserBase
		{
			...
			{
				++_total;
				CurrentAverage += (val - CurrentAverage) / _total;
			}

			public override object Result => CurrentAverage;
		...
		{
			readonly Dictionary<uint, NumericSummariser>
				_curr = new Dictionary<uint, NumericSummariser>();
			...
			public override object Result =>
				WeightedIndexList.Create(_curr.Select(kv =>
					new WeightedIndexList.WeightedIndex
					{
						Index = kv.Key, Weight = Convert.ToSingle(kv.Value.CurrentAverage)
					}).ToArray());
		}
		...
		class VectorSummariser : SummariserBase
		{
			...
			public void Add(FloatVector vector)
			{
				if (vector.Data == null)
					return;
				if (!_summarisers.Any())
					_summarisers.AddRange(Enumerable.Range(0, vector.Size).
						Select(i => new NumericSummariser(i)));
			...
			public override object Result
			{
				get
				{
					return FloatVector.Create(_summarisers.Select(s => Convert.ToSingle(s.CurrentAverage)).
						ToArray());
				}
			}
		}
		...
		class MatrixSummariser : SummariserBase
		{
			...
			{
				if (!_summarisers.Any())
					_summarisers.AddRange(Enumerable.Range(0, matrix.RowCount).
						Select(i => new VectorSummariser(i)));
				var rows = matrix.Row;
			...
			public override object Result
			{
				get
				{
					return FloatMatrix.Create(_summarisers.Select(r => (FloatVector)r.Result).ToArray());
				}
			}
		}
		...
		class TensorSummariser : SummariserBase
		{
			...
			public override void Add(IRow row)
			{
				...
				if (!_summarisers.Any())
					_summarisers.AddRange(Enumerable.Range(0, tensor.Depth).
						Select(i => new MatrixSummariser(i)));
			...
			public override object Result
			{
				get
				{
					return FloatTensor.Create(_summarisers.Select(r => (FloatMatrix)r.Result).ToArray());
				}
			}
		}
