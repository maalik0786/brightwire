...
namespace BrightWire
{
	public static partial class ExtensionMethods
	{
			...
			for (var i = 0; i < numIterations; i++)
			{
				...
				if (++count == testCadence)
				{
					if (engine.Test(testData, errorMetric, 128,
						...
						onImprovement != null)
					{
						var bestModel = new GraphModel { Graph = engine.Graph };
		...
		public static IReadOnlyList<(IRow Row, string Classification)> Classifiy(
			this IRowClassifier classifier, IDataTable dataTable)
		{
		...
		internal static IGradientDescentOptimisation CreateGradientDescentOptimisation(
			this GraphFactory factory, BinaryReader reader)
		{
		...
		public static IReadOnlyList<FloatVector[]> OrderSequentialOutput(
			this IReadOnlyList<ExecutionResult> results)
		{
			...
			return ret.GroupBy(d => d.Key.RowIndex).
				...
				OrderBy(d => d.Item1).Select(d => d.Item2).ToList();
		}
	}
}