...
namespace BrightWire.Helper
{
	...
	class DataTableBuilder : IHaveColumns
	{
		...
		internal class Column : IColumn
		{
			...
			bool _Validate(object value)
			{
				...
				case ColumnType.Matrix:
					return value is FloatMatrix matrix && _ValidateDimension(matrix.RowCount, DimensionY) &&
						_ValidateDimension(matrix.ColumnCount, DimensionX);
				case ColumnType.String:
					return value is string;
				case ColumnType.Tensor:
					return value is FloatTensor tensor && _ValidateDimension(tensor.Depth, DimensionZ) &&
						_ValidateDimension(tensor.RowCount, DimensionY) &&
						_ValidateDimension(tensor.ColumnCount, DimensionX);
				...
				if (value != null)
				{
					if (validate && !_Validate(value))
						throw new ArgumentException(
							$"Invalid type for column {Index} ({_type.ToString()}): {value}");
		...
		public DataTableBuilder(IEnumerable<IColumn> columns, bool validateOnAdd = true,
			int blockSize = 1024) : this(validateOnAdd, blockSize)
		{
		...
		public IColumn AddTensorColumn(int rows, int columns, int depth, string name = "",
			bool isTarget = false)
		{
			var ret = _AddColumn(ColumnType.Matrix, name, isTarget);
			ret.DimensionX = columns;
		...
		internal void WriteMetadata(Stream stream)
		{
			...
			writer.Write(BlockSize); // write the block size
