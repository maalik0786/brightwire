	...
	class BernoulliNaiveBayesClassifier : IIndexListClassifier
	{
		class Classification
		{
			...
			public double GetScore(HashSet<uint> documentSet)
			{
				...
				ret += (missingCount * _class.MissingProbability) +
					(inverseMissingCount * _class.InverseMissingProbability);
				return ret;
			}
		}
		...
		{
			_classification =
				model.ClassData.Select(c => new Classification(model.Vocabulary, c)).ToList();
		}
		...
		{
			var indexColumn = row.Table.Columns.First(r =>
				r.Type == ColumnType.IndexList || r.Type == ColumnType.WeightedIndexList);
			IndexList indexList;
