...
namespace BrightWire.Bayesian
{
	...
	class NaiveBayesClassifier : IRowClassifier
	{
		...
		readonly List<Tuple<string, double, List<IProbabilityProvider>>> _classProbability =
			new List<Tuple<string, double, List<IProbabilityProvider>>>();
		...
		{
			return _Classify(row).OrderByDescending(kv => kv.Value).Take(1).
				Select((kv, i) => (kv.Key, 1f)).ToList();
		}
	}
}