...
namespace BrightWire.Bayesian.Training
{
	...
	internal class MarkovModelTrainer2<T> : IMarkovModelTrainer2<T>
	{
		...
		public MarkovModel2<T> Build()
		{
			...
			{
				var transitions = item.Value.GroupBy(v => v).Select(g => Tuple.Create(g.Key, g.Count())).
					Where(d => d.Item2 >= _minObservations).ToList();
				var total = (float)transitions.Sum(t => t.Item2);
				if (total > 0)
				{
					ret.Add(new MarkovModelObservation2<T>(item.Key.Item1, item.Key.Item2,
						transitions.Select(t =>
							new MarkovModelStateTransition<T>
							{
								NextState = t.Item1, Probability = t.Item2 / total
							}).ToList()));
				}
			}

			return new MarkovModel2<T> { Observation = ret.ToArray() };
		}
		...
		public void SerialiseTo(Stream stream)
		{
			...
			var tempDictionary =
				_data.ToDictionary(kv => Tuple.Create(kv.Key.Item1, kv.Key.Item2), kv => kv.Value);
	...
	internal class MarkovModelTrainer3<T> : IMarkovModelTrainer3<T>
	{
		...
		public MarkovModel3<T> Build()
		{
			...
			foreach (var item in _data)
			{
				...
				if (total > 0)
				{
					ret.Add(new MarkovModelObservation3<T>(item.Key.Item1, item.Key.Item2, item.Key.Item3,
						transitions.Select(t =>
							new MarkovModelStateTransition<T>
							{
								NextState = t.Item1, Probability = t.Item2 / total
			...
			return new MarkovModel3<T> { Observation = ret.ToArray() };
		}
		...
		public void SerialiseTo(Stream stream)
		{
			...
			// serialise as Tuple as ValueTuple is not binary serializable
			var tempDictionary =
				_data.ToDictionary(kv => Tuple.Create(kv.Key.Item1, kv.Key.Item2, kv.Key.Item3),
					kv => kv.Value);
			formatter.Serialize(stream, tempDictionary);
		}
	}
