...
namespace BrightWire.Bayesian.Training
{
	...
	static class NaiveBayesTrainer
	{
		...
		public static NaiveBayes Train(IDataTable table)
		{
			...
			var classBasedFrequency = classInfo.DistinctValues.Select(cv =>
				Tuple.Create<string, IRowProcessor>(cv.ToString(),
					new FrequencyAnalysis(table, classColumnIndex)));
			var frequencyAnalysis =
				new ClassificationBasedRowProcessor(classBasedFrequency, classColumnIndex);
			...
			return new NaiveBayes { Class = classList };
		}
	}
}