...
namespace BrightWire.Bayesian.Training
{
	...
	internal class MultinomialNaiveBayesTrainer
	{
		...
		readonly Dictionary<string, List<IndexList>> _documentClass =
			new Dictionary<string, List<IndexList>>();
		...
		public MultinomialNaiveBayes Train()
		{
			...
			foreach (var item in _documentClass)
			{
				...
				foreach (var word in allClassToken.GroupBy(d => d).
					Select(d => Tuple.Create(d.Key, d.Count())))
				{
			...
			return new MultinomialNaiveBayes { ClassData = ret.ToArray() };
		}
	}
}