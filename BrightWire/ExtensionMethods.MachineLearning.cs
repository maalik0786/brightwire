...
namespace BrightWire
{
	public static partial class ExtensionMethods
	{
		...
		/// <param name="s"></param>
		public static IRandomProjection CreateRandomProjection(this ILinearAlgebraProvider lap,
			int fixedSize, int reducedSize, int s = 3)
		{
		...
		public static LogisticRegression TrainLogisticRegression(this IDataTable table,
			ILinearAlgebraProvider lap, int iterations, float learningRate, float lambda = 0.1f,
		...
		public static ILogisticRegressionTrainer CreateLogisticRegressionTrainer(this IDataTable table,
		...
		public static List<MarkovModelStateTransition<T>> GetTransitions<T>(
			this Dictionary<MarkovModelObservation2<T>, List<MarkovModelStateTransition<T>>> model,
			T item1, T item2)
		{
			var observation = new MarkovModelObservation2<T> { Item1 = item1, Item2 = item2 };
		...
		/// <returns>The list of state transitions or null if nothing was found</returns>
		public static List<MarkovModelStateTransition<T>> GetTransitions<T>(
			this Dictionary<MarkovModelObservation3<T>, List<MarkovModelStateTransition<T>>> model,
			T item1, T item2, T item3)
		{
			var observation =
				new MarkovModelObservation3<T> { Item1 = item1, Item2 = item2, Item3 = item3 };
			if (model.TryGetValue(observation, out List<MarkovModelStateTransition<T>> ret))
				return ret;
			return null;
		}
		...
		public static IReadOnlyList<IReadOnlyList<IVector>> NNMF(this IReadOnlyList<IVector> data,
		...
		/// <returns>A model that can be used for classification</returns>
		public static DecisionTree TrainDecisionTree(this IDataTable data, int? minDataPerNode = null,
			int? maxDepth = null, double? minInformationGain = null, int? maxAttributes = null)
		{
		...
		/// <param name="data">The training data</param>
		/// <returns>A model that can be used for classification</returns>
		public static MultinomialNaiveBayes TrainMultinomialNaiveBayes(
			this IReadOnlyList<(string Classification, IndexList Data)> data)
		{
		...
		public static MultinomialNaiveBayes TrainMultinomialNaiveBayes(this IDataTable table)
		{
			...
			var data = table.Map(row => (row.GetField<string>(targetColumnIndex),
				row.GetField<IndexList>(indexListColumn.Index)));
		...
		/// <param name="data">The training data</param>
		/// <returns>A model that can be used for classification</returns>
		public static BernoulliNaiveBayes TrainBernoulliNaiveBayes(
			this IReadOnlyList<(string Classification, IndexList Data)> data)
		{
		...
		public static ILinearRegressionTrainer CreateLinearRegressionTrainer(this IDataTable table,
			ILinearAlgebraProvider lap)
		{
		...
		/// <returns></returns>
		public static string GetBestClassification(
			this IReadOnlyList<(string Label, float Weight)> classifications)
		{
		...
		public static IRowClassifier ConvertToRowClassifier(
			this ILogisticRegressionClassifier classifier, IReadOnlyList<int> attributeColumns,
			string negativeLabel = "0", string positiveLabel = "1")
		{
			return new LogisticRegressionClassifierAdapter(classifier, attributeColumns, negativeLabel,
				positiveLabel);
		}
	}
}